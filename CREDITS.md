...
- **Iridescence (thin-film Fresnel)** — Laurent Belcour, Pascal Barla, "A Practical Extension to Microfacet Theory for the Modeling of Varying Iridescence," ACM Transactions on Graphics 36(4) (SIGGRAPH 2017). <https://hal.archives-ouvertes.fr/hal-01518344/document>. Implemented in `@vitrum/pt-webgl2/src/glsl/shader/bsdf/iridescence_functions.glsl.js` (GLSL original) and ported to WGSL in `@vitrum/pt-webgpu/src/wgsl/pathTrace/bsdf.wgsl.ts` (H52 — thin-film F0 modification before Cook-Torrance evaluation).
- **Sheen BRDF (Charlie NDF)** — Alejandro Estevez, Christopher Kulla, "Production Friendly Microfacet Sheen BRDF," SIGGRAPH 2017 Physically Based Shading course; visibility term from David Neubelt, Matt Pettineo, "Crafting a Next-Gen Material Pipeline for The Order: 1886," SIGGRAPH 2013. Implemented in `@vitrum/pt-webgpu/src/wgsl/pathTrace/bsdf.wgsl.ts` (H52 — evaluation-only sheen lobe for velvet/fabric) and as an approximate shade-owned scalar lobe in `@vitrum/walkaround-hybrid/src/shaders/ggxBrdf.wgsl.ts`.
- **glTF material extensions** — Khronos Group: KHR_materials_clearcoat, KHR_materials_sheen, KHR_materials_iridescence (glTF 2.0 extension specifications, <https://github.com/KhronosGroup/glTF>). These specs define the clearcoat (additive GGX at IOR 1.5), sheen (Charlie distribution retro-reflective highlight), and iridescence (thin-film F0 override) lobe semantics implemented in H52 for `@vitrum/pt-webgpu`; scalar clearcoat and sheen are also approximated in `@vitrum/walkaround-hybrid` shade-owned lighting paths.
...
- **NRC (Neural Radiance Caching)** — Thomas Müller, Fabrice Rousselle, Jan Novák, Alexander Keller, "Real-time Neural Radiance Caching for Path Tracing," ACM TOG 40(4) (SIGGRAPH 2021) — §4 (input encoding + the small MLP), §5 (self-training scheme + the path-spread cache-termination heuristic `a(x) > c·a₀`). Drives the opt-in `HybridEngineOptions.nrcEnabled` subsystem: the fused/tiled MLP train kernel (`walkaround-hybrid/src/neural/nrc/wgsl/fusedMlp.wgsl.ts` + `fusedMlpTrainer.ts`), the input-encoding CPU oracle + WGSL forward/backward (`walkaround-hybrid/src/neural/nrc/nrcEncoding.ts` + `wgsl/nrcEncoding.wgsl.ts`), the spread-termination predicate (`spreadTermination.ts` + `wgsl/spreadTermination.wgsl.ts`), the **LIVE cache query** — an inline single-sample MLP forward + encode that replaces the DDGI suffix estimate at the gi-ris reconnection vertex when §5 spread fires (`wgsl/nrcQuery.wgsl.ts` + the NRC gi-ris variant `shaders/risGiNrc.wgsl.ts`) — and the **host-owns-cadence self-training driver** (`nrcSubsystem.ts`: per-frame record gather → one MLP `trainStep` → a hash-grid TABLE training step, so the MLP AND the multiresolution encoding both learn). The query + train passes are **compile-time-gated** on `nrcEnabled` (the gi-ris pipeline gains a 5th `@group(4)` NRC group ONLY when on; OFF is byte-for-byte the pre-NRC pipeline — the GRIS-class-regression discipline, guarded by `__tests__/nrcStructuralGate.test.ts`). **NRC is a BIASED cache** (a learned approximation of the path suffix, not an unbiased estimator) — acceptance is perceptual closeness + faster convergence, not equal converged mean. See `HARDWARE-VALIDATION-NEEDS.md` V20.
- **Multiresolution hash-grid encoding (Instant-NGP)** — Thomas Müller, Alex Evans, Christoph Schied, Alexander Keller, "Instant Neural Graphics Primitives with a Multiresolution Hash Encoding," ACM TOG 41(4) (SIGGRAPH 2022) — §3 (L hashed feature grids, trilinear interpolation, concat; Eq. 4 spatial hash with primes 1 / 0x9E3779B1 / 0x30034BB7), §4 (the trainable backward: collisions onto a table row accumulate gradients; the embedding LR ≈ 10× the MLP LR). The trainable positional encoding of the NRC cache-query vertex: CPU oracle (`walkaround-hybrid/src/neural/nrc/nrcEncoding.ts` `hashGridForward`/`hashGridBackward`) + the GPU forward (`wgsl/nrcEncoding.wgsl.ts` / `nrcQuery.wgsl.ts`) + the **dispatched** encode-backward scatter (`wgsl/nrcEncodeBackward.wgsl.ts`, a standalone compute kernel that reads the MLP's `dL/dX` and scatters `weight·dL/dfeature` into the per-level gradient tables via i32 fixed-point atomics, then a separate table Adam in `nrcSubsystem.ts`). The MLP backward (`wgsl/fusedMlp.wgsl.ts`) now emits the layer-0 input gradient `dL/dX` to feed this. The tables LEARN (no longer frozen at random init).
