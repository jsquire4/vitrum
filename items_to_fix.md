...
### F-RC1 — RC merged-mode GPU traversal is unreliable on MULTI-NODE trees (discovered 2026-06-04)
...
### F-TLAS1 — ReSTIR-TLAS `bvhIndex.xyz` vertex lanes packed STRIDE-3 from a STRIDE-4 buffer (CODE-CONFIRMED 2026-06-04)

- **Severity:** potentially HIGH (production geometry), but runtime impact UNRESOLVED — see the puzzle below. Surfaced by the ReSTIR-material decouple agent; **all three code links independently verified by the lead.**
...
- **THE PUZZLE (must resolve before fixing):** a geometry corruption this broad should make multi-mesh ReSTIR-TLAS renders visibly wrong, yet the engine isn't obviously broken and the T1 smoke passes. Either (a) the goldens were captured WITH the bug (non-regression vs garbage — which would also indict the smoke's correctness value), (b) the tested scenes route through merged not TLAS, or (c) a masking factor (e.g. `bvhIndicesStride3` is the buffer actually bound, not `bvhIndex` — though the `.w` material read argues against that). Resolve this FIRST — it determines whether this is catastrophic or latent-dead-data.
