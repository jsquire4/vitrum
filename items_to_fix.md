...
- **RESIDUAL (the only open part) — 5 multi-mesh hybrid-TLAS reference renders captured ≥2026-05-26 still encode the bug; re-capture needs the Playwright+Chrome benchmark-runner on a HARDWARE GPU (not drivable on the wsl-gpu/lavapipe deno worker):** `tools/reference-renders/PR-hybrid/tlas-on/PR-hybrid-tlas-10-inst.png`, `PR-hybrid/200k-static/PR-hybrid-200k-static.png`, `PR-hybrid/material-edit/PR-hybrid-material-churn.png` (all via `npm run benchmark:pr-hybrid-refs`), and `session-20260527/hero-viewer-realtime.png` + its byte-identical `baseline/hero-viewer-realtime.png` (via `npm run capture:refs`). VERIFIED NOT affected: all pt-webgl/pt-webgpu refs (own packers), all `W8-rc-*` (walkaround-rc path), and anything captured < 2026-05-26.
