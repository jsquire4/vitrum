...
>
> **PT-WEBGL2 A/B COVERAGE MAP (2026-06-09) — "what's left before dropping the fork".** Expanded the fork-vs-native A/B (`wsl-gpu/webgl2-capture/run-ptwebgl2-vs-fork.mjs`, `SCENE=cornell|glass|envmap`) across hard scenes: **(1) Diffuse GI — FAITHFUL** (meanLum 1.0023, per-channel <0.5%). **(2) Glass/transmission — FAITHFUL** (Cornell + glass sphere, refraction/Fresnel/IOR/attenuation; 96spp meanLum 1.0048, per-channel ~1%, channels straddle 1.0 = noise). **(3) Env-map/IBL — math FAITHFUL after a real fix** (`43a5b34`: `envMapInfo.totalSum` was never set → env-sampling GLSL early-outed on `totalSum==0` → IBL silently rendered fully black; now lit, 240spp meanLum 0.984 converging, visibility split identical to fork on a high-variance small-sun scene). **Two OPEN coverage gaps remain for fork parity:** **(G2) IBL data-bridge** — `three-bindings` `resolveEnvironment` hands `hdri` as a `THREE.Texture` (backend-specific by core's `EnvironmentMapRef = unknown` design — the fork-wrapping `@vitrum/pt-webgl` consumes it directly), but THREE-free backends need a raw `{width,height,data}` payload, so IBL is silently dropped through the THREE on-ramp (empirically: unpatched native env-scene = meanLum 0). Fix is an architecture choice (host-side THREE-texture→payload bridge utility, OR pt-webgl2 duck-types `.image.data` incl. RGBA-stride/HalfFloat — but pt-webgl2 stays THREE-free; changing `resolveEnvironment`'s output would break `@vitrum/pt-webgl`). **(G3) Textures unimplemented** — `materialsTexture.ts` hardcodes every texture id to `NO_TEXTURE=-1` (no atlas yet, explicit in-code), so textured materials render with flat base color; needs a texture-atlas packer + the attributes UV path. Math fidelity for the implemented paths is proven; G2/G3 are feature-completeness, not math gaps.
>
> **OPEN — pt-webgl2 G1: gNormalDepth aux g-buffer diverges under the running-average regime.** `composeTraceGlsl.ts:978` writes `gNormalDepth = vec4(normalEnc, linearDepth)` — depth (>1) sits in `.a`. The 'normal' accum regime uses fixed-function blend `dst = src·srcA + dst·(1−srcA)` where WebGL2's `srcA` is **each attachment's own output alpha**. Attachment 0 (color, α=opacity≤1) averages correctly; `gAlbedo` (α=1.0) overwrites correctly; but `gNormalDepth`'s α=depth>1 makes the recurrence diverge geometrically → garbage/NaN (observed −46175 at 8spp, NaN at 128spp). **Radiance is unaffected** — this is a denoiser-aux path nothing consumes yet. Fix is an architecture choice (NOT a one-liner): (a) re-encode g-buffer so `.a`=1.0 and depth moves into a free channel — touches the `shared-denoisers` normal/depth read contract; (b) `OES_draw_buffers_indexed` → `disablei(BLEND, 1/2)` so aux attachments overwrite (the g-buffer is primary-hit/sample-invariant, so "latest sample" is correct) — adds an extension dependency + fallback; (c) a separate non-blended g-buffer pass. Decide approach before wiring a pt-webgl2 denoiser.
