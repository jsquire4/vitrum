...
async function main(): Promise<void> {
  ...
  function loop(): void {
    ...
    // -------- OIDN (manual dispatcher) — regression-baseline path --------
    // This block drives OIDN manually behind ?vitrumDisplay=oidn:
    // readAccumulationRgbFloat → denoiseFinal → writeTonemappedRgbToCanvas, with
    // its own oidnStarted/oidnDisplaySucceeded state machine.
    //
    // pt-webgl ALSO ships a built-in OIDN dispatcher (W11, merged) behind the typed
    // `oidn: { modelUrl }` engine option + `denoiser: 'oidn-final'`, polled via
    // `engine.getDenoisedFrame()`. This manual block is kept intentionally as the
    // standalone regression baseline — it exercises the @vitrum/shared-denoisers
    // `denoiseFinal` path directly, independent of the in-engine dispatcher.
    //
