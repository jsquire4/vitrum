...
const FRAME_FIELDS = [
  ...
  // H14-E: HDRI intensity in its own slot (31), separate from environmentSun.w.
...
// `struct RCLightBuffer` header: count (u32) + 3 × u32 pad = 16 bytes.
...
// Mirrors `struct CascadeUniforms` in probeRayCast.wgsl.ts (40 words = 160 bytes).
...
function generateRC() {
  ...
  // RCLightBuffer = header (16 bytes) + array<RCLight, 16>; total 1040 bytes.
  const RC_LIGHTS_MAX = 16;
  const bufferBytes = header.byteSize + RC_LIGHTS_MAX * light.byteSize;
  return (
    ...
    `export const RC_LIGHTS_MAX = ${RC_LIGHTS_MAX};\n\n` +
    `/** Byte size of the full \`RCLightBuffer\` GPU allocation (1040 bytes). */\n` +
    `export const RC_LIGHTS_BUFFER_BYTES = ${bufferBytes};\n\n` +
    `/** Byte offsets within the \`RCLightBuffer\` header. */\n` +
    emitOffsetObject('RCLightBufferHeaderOffset', { count: header.offsets.count }) +
    `\n` +
