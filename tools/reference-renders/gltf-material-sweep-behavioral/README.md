...
`manifest.json` and `tools/gltf-material-sweep/proofs.mjs` are the shared
metadata used by the CPU sweep and behavioral gate. Keep them in sync with:

```sh
npm run gltf-material-proof-check
```
