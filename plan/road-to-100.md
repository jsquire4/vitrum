...
| ~~Morph `TEXCOORD_0/1` contract policy~~ ✅ DONE | `SkinnedMeshPrimitive.morphTargetUvs` / `.morphTargetUv1s`, `skinSolver.ts`, `gltfToScene.ts`, `featureReport.ts`, `sceneController.ts`, `skinnedMeshIngestion.test.ts`, `pt-webgl2/src/scene/solveSkinPrimitives.ts`, `skinSolve.test.ts`, `mutationMatrix.test.ts` | glTF morph-target UV deltas for the two UV lanes the core Scene contract actually carries now import, preflight as supported when the matching base UV stream exists, blend through the shared CPU skin solver, propagate through animation patches, and reach pt-webgpu/pt-webgl2/walkaround attribute uploads. Unsupported cases are now narrower and explicit: missing base `TEXCOORD_0/1` streams or `TEXCOORD_2+` morph deltas still emit `ignored-morph-target-texcoord` diagnostics plus `morphTargetTexcoords=unsupported` compatibility issues. |
