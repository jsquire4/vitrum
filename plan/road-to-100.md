...
`roughnessMap`/`metallicMap` (2026-06-14) now have real walkaround paths for
...
BRDF roughness/metallic from the glTF G/B channels. `CONSUMED_MATERIAL_FIELDS`
and the core promise ledger now grade walkaround `baseColorMap`,
`roughnessMap`, and `metallicMap` as `approximate`. They are deliberately not
...
|-----------|---------|-------|
| Atlas build | ✅ SECOND SLICE: `pipeline/materialTextureAtlas.ts` | `baseColorMap` raw/DataTexture handles are inverse-sRGB decoded into linear RGBA32F array layers; `roughnessMap` and `metallicMap` are packed as linear scalar-map layers and sampled from glTF G/B channels. Remaining maps still need atlas rows/channel policy. |
...
| Material index per tri | ✅ SECOND SLICE: metadata texture keyed by triangle index | `baseColorMap`, `roughnessMap`, and `metallicMap` use `triangleMaterialIds` at pack time; scalar lanes stay as fallback when no readable map exists. |
...
| Ledger | ✅ SECOND SLICE: `WALKAROUND_MATERIALS`, `CONSUMED_MATERIAL_FIELDS` | `baseColorMap`, `roughnessMap`, and `metallicMap` promoted to `approximate` with tests. Remaining maps remain unsupported until each has shader consumption. |
