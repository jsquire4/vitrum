...
Walkaround **100%** and arbitrary glTF **100%** are not the same: arbitrary glTF routes rich assets to PT backends via the planner; walkaround 100% still means permanent `unsupported` for spectral/volume/layered domains with explicit rejection, while displacement is only vertex-level approximate when CPU-readable.
