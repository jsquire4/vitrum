...
> (`ior`/`transmission`/`thickness`/`attenuationDistance`) from visibility tails
...
> Latest transport follow-up: `attenuationDistance` now joins `transmission` and
> `thickness` as a finite-difference inverse parameter with a structured
...
| Adjoint | `adjointPass.wgsl.ts`, `pathTraceAdjoint.wgsl.ts`, `inverseSession.ts` | ◻/✅ Scoped path replay now consumes `req.samples`, replays the frozen inverse sample sequence, and covers opaque direct-light/unlit-primary fields including baseColor, roughness, metallic, emissive/emissiveIntensity, specular, clearcoat, sheen, iridescence, anisotropy, their documented local map factors, deterministic direct emitter `color`/`intensity`, and delta directional/point/spot/stochastic area-measure rect/disc/mesh-area direct lights. Finite-difference inverse sessions now also admit scalar transport/visibility/map controls (`transmission`, `thickness`, `attenuationDistance`, `opacity`, `alphaCutoff`, `normalScale`, `bumpScale`, `clearcoatNormalScale`, `aoMapIntensity`, `lightMapIntensity`, `envMapIntensity`) with structured path-replay downgrades. Still OPEN for full-path parity: alpha-map/alpha-visibility adjoints, analytic normal/bump/light/transmission/thickness/attenuation-distance/displacement and clearcoat-normal replay, layered/volume/spectral material domains, environment/soft-sun light terms, exact texel-PDF mesh emission, forward light-selection MIS parity, indirect paths, GPU inverse-fit recaptures, and full material-lobe A/B proofs. |
