...
> finite-difference optimization for renderer-consumed scalar normal controls
> (`clearcoatNormalScale`) while requested path replay downgrades it explicitly
> until analytic clearcoat-normal terms are mirrored; top-level `normalMap` /
> `normalScale` and `bumpMap` / `bumpScale` are now mirrored in the scoped
> direct-light adjoint path, including the forward shader's authored/generated
> tangent.xyzw handedness before UV-gradient fallback. `normalScale` still
> downgrades when a bump map is present because the chained derivative through
> the bump tangent frame is not implemented.
...
> path replay. Zero-intensity mapped mesh emitters still downgrade because the
> forward packer drops zero-power mesh triangles and therefore has no replayable
> source range.
