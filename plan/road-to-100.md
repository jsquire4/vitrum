...
> (`normalScale`, `bumpScale`, `clearcoatNormalScale`, `aoMapIntensity`,
> `lightMapIntensity`, `envMapIntensity`) while requested path replay downgrades
> them explicitly until analytic normal/map-scale terms are mirrored.
...
> `kind:'vec2'` inverse sessions and map-free ranges stay on scoped path replay
> by differentiating sampled thin-film thickness and chaining it to min/max
> endpoints. Authored `iridescenceThicknessMap` still downgrades until
> texture-texel range gradients are mirrored.
...
| Adjoint | `adjointPass.wgsl.ts`, `pathTraceAdjoint.wgsl.ts`, `inverseSession.ts` | ◻/✅ Scoped path replay now consumes `req.samples`, replays the frozen inverse sample sequence, and covers opaque direct-light/unlit-primary fields including baseColor, roughness, metallic, emissive/emissiveIntensity, specular, clearcoat, sheen, iridescence, map-free `iridescenceThicknessRange`, anisotropy, their documented local map factors, deterministic direct emitter `color`/`intensity`, and delta directional/point/spot/stochastic area-measure rect/disc/mesh-area direct lights. Finite-difference inverse sessions also admit transport/visibility/map controls (`transmission`, `thickness`, `attenuationColor`, `attenuationDistance`, `opacity`, `alphaCutoff`, `normalScale`, `bumpScale`, `clearcoatNormalScale`, `aoMapIntensity`, `lightMapIntensity`, `envMapIntensity`) with structured path-replay downgrades. Still OPEN for full-path parity: alpha-map/alpha-visibility adjoints, analytic normal/bump/light/transmission/thickness/attenuation-color/attenuation-distance/displacement, clearcoat-normal, and mapped iridescence-thickness replay, layered/volume/spectral material domains, environment/soft-sun light terms, exact texel-PDF mesh emission, forward light-selection MIS parity, indirect paths, GPU inverse-fit recaptures, and full material-lobe A/B proofs. |
| Present | `present.wgsl.ts` tonemap only — no BSDF | N/A |

**Footgun:** Fixing megakernel only used to leave BDPT/SPPM/ReSTIR-PT wrong for glTF clearcoat scenes on specialty paths. The clearcoat-normal map now reaches the megakernel, BDPT light-side surface vertices, and ReSTIR-PT reservoir/resolve/source-PDF/suffix-Lo paths; BDPT's light-side material domain now also applies layer tint/roughness, thin-film reflect tint, Cauchy IOR, and spectral reflectance scalar. ReSTIR-PT now has a deterministic CPU/static specialty-lobe identity fixture, map-free scalar clearcoat/sheen/iridescence controls have scoped direct-light adjoint partials, and map-free iridescence thickness ranges have path-replay inverse fitting, but the remaining class still includes clearcoat maps/normal maps, sheen maps, mapped iridescence thickness gradients, anisotropy inverse-adjoint derivatives, plus GPU material-furnace/reference-render promotion, not just missed function calls.
