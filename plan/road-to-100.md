...
| ID | Item | Where | State now | Done = | Effort |
|----|------|-------|-----------|--------|--------|
| **B1** | ✅ **DONE (Wave A + R8-B tail) — metals/glossy lit; glass refracted GI done** | `shaders/risGi.wgsl.ts`, `materialDecode.wgsl.ts`, `shade.wgsl.ts` | **Wave A:** per-tri `bvh_material` r32uint texture; metals get DI + analytic NEE + specular indirect. **R8-B tail (2026-06-10):** glass primaries get a refracted GI reservoir via 1-interface Snell walk (IOR=1.5); shade `lo_transmittedGI` weights by Fresnel-T + Beer tint. | NRC xsRough wiring; per-tri IOR pack slot (B1-ior-per-tri). | done |
...
- **B1 tail — glass refracted GI: DONE (2026-06-10, R8-B item 9).** 1-interface refraction walk in `risGi.wgsl.ts`: when the primary hit is glass, Snell refract the camera ray (fixed IOR_GLASS=1.5, no per-tri IOR in current pack layout — `bvh_material` stores rough+metal only; per-tri IOR deferred, plan note "B1-ior-per-tri"), walk to the first non-glass diffuse surface (straight-through approximation for up to 2 additional interfaces), build the cosine-hemisphere GI reservoir there. Temporal/spatial reuse stores and compares the post-glass surface's xv/nv — geometrically consistent across reuse passes. Shade: `lo_transmittedGI` reads the reservoir, weights by Fresnel scalar transmission (Schlick, F0=0.04) × Beer-Lambert bvh_beer tint, joins the un-demodulated direct channel (not AO-modulated, not albedo-demodulated). Multi-interface rough-glass out of scope. Behavioral gate: `wh/glass-gi` (glass pane + ceiling emitter; asserts non-black). Render-changing for glass scenes → V28-B recapture in R8-C.
