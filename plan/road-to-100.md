...
- **D2 — Silent data drops** ◻: `three-bindings/src/index.ts:270` drops `THREE.Points/Line/LineSegments/Sprite/BatchedMesh/LOD` with **no warning** (lights get `warnOnce`, geometry types get nothing); pt-webgpu `MaterialTextureArray.warnings` now route through `UploadedSceneBuffers.warnings` / the engine structured warning path (2026-06-12), but heterogeneous texture sizes still copy into a max-size layer with **wrong UVs** and only a compatibility warning rather than a true per-layer UV-fit/atlas solution. **Fix remaining: warn-or-handle the THREE drops; implement true texture sizing/UV-fit or atlas behavior. Effort: S–M.**
...
| **Texture wrap modes (ADDED 2026-06-12 — verified hole)** | ~~`TextureRef.wrapS/wrapT` in core (`repeat`/`clamp`/`mirror`) + adapter reads `gltf.samplers`~~ ✅ API DONE (`material.ts`, `textures.ts`, `gltfTextureSweep.test.ts`, `gltfAssetApi.test.ts`, `texturePipeline.ts`). Still open: backend honor. pt-webgpu material texture array uses one repeat sampler; pt-webgl2 / walkaround need sampler/wrap audit and shader/per-layer strategy where one array sampler cannot vary by texture. | API no longer drops sampler semantics and `textureDecodeReport` exposes wrap modes. Runtime parity for clamp/mirror/repeat remains required before visual Khronos-sweep signoff; KHR_texture_transform scale>1 can still render incorrectly on backends that force one wrap policy. |
