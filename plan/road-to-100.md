...
- **D3 — Contract material gaps** ✅(contract)/◻: `MaterialSpec` (core) has no slot for `specularColor`/`specularIntensity` (KHR_materials_specular content silently lost), `bumpMap`, `displacementMap`, `lightMap`, `envMapIntensity`. Add core fields + bind in three-bindings + consume in backends. **Effort: M.**
...
- **D7 — SVGF texture allocation** ◻: audit P2 says ~10 full-res SVGF textures allocated even when denoiser≠svgf-real; the pipeline reader says they collapse to 1×1 when off (`createSvgfFrameResources.ts:63`). **Conflicting — verify first**, then fix if real. **Effort: S.**
- **D8 — fork lint/smoke red** ◻: `three-gpu-pathtracer` `shader-smoke-check` fails (stale gate vs the corrected per-material SSS). **Effort: S.**
