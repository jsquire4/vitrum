...
> `npm run road-to-100-validation-status`. The source check now also runs
> `npm run road-to-100-source-gap-scan`, which classifies every production
> TODO/FIXME/stub/not-implemented marker and fails on any new unclassified
> marker before it can become another rediscovered ghost gap. Both gates are
> wired into `npm run road-to-100-source-check` and therefore
> `npm run proof-check`. The queue makes the current state explicit: no active
> implementation rows, a finite validation/proof/provisioning queue with
> commands and artifacts, and separate future-contract rows for displacement,
> true transparent GI transport, native point/line primitives, arbitrary UV
> arrays, and native instanced skinned/morphed glTF.
> **2026-06-22 future-contract guard follow-up:** each future-contract row now
> carries `codeNowBounded:false` plus concrete decision blockers, and
> `npm run road-to-100-validation-status` fails if those blockers disappear.
> This prevents the same non-bounded physics/API programs from being re-opened
> as ordinary code gaps without an explicit contract-promotion decision.
...
> and capped mesh-area targets on path replay. Current inverse direct-light follow-up:
> `createPTEngine_WebGPU()` now packs `directLightingMode:"summed-expectation"`
> during inverse readbacks, and both full/lite kernels sum eligible direct-light
> candidates in that scoped replay baseline while preserving sampled one-of-N
> selection for ordinary renders. Real multi-light inverse sessions no longer
> downgrade for light selection alone; hook contexts that still report
> `directLighting:"sampled-selection"` keep the structured
> `path-replay-unsupported-light-selection` downgrade. Remaining scoped inverse
> tails are exact texel-PDF mesh emission promotion, environment/indirect
> transport, alpha/visibility transport, and unreplayed receiver material domains,
> which still downgrade with structured diagnostics.
...
> (`opacity`/`alphaCutoff`) and geometry tails
> (`displacementScale`/`displacementBias`) via contract-level diagnostic codes.
> The scalar displacement controls are finite-difference inverse parameters now;
> they are not a promotion of native displacement rendering/path replay. Latest RC
...
> **2026-06-22 pt-webgpu soft-directional follow-up:** full-tier pt-webgpu now
> routes authored directional `angularDiameter` through the scoped in-medium NEE
> branch and ReSTIR-PT suffix direct-light replay, using the same signed
> shadow-disabled cone decode and cone sampler as surface direct lighting.
> Render/A-B recapture remains validation work.
...
> unsupported-material warnings retain the aggregate `fields` list while also
> reporting exact `primitiveIds` plus per-primitive `primitiveFields`, matching
> the mutation-path diagnostic specificity. Displacement has since moved to
> approximate shared-BVH vertex displacement with scene-pack/fallback diagnostics.
> **2026-06-22 walkaround warning-authorship follow-up:** walkaround
> `setScene()` and `updatePrimitive()` unsupported-material warnings now also
> carry `details.primitiveFields[]`, so permanent unsupported rows still fail
> honestly but hosts can locate the exact primitive that authored each parked
> specialty field.
...
> Same-day sampler bridge follow-up: `loadGltfForEngine()` now classifies
...
> Sparse auxiliary-stream follow-up: optional primitive attributes, morph-target
> deltas, `EXT_mesh_gpu_instancing` attributes, and skin inverse-bind matrices
> now surface malformed sparse patch storage as approximate/degraded
> compatibility issues before import. Required POSITION/index sparse corruption
> remains a hard unsupported primitive blocker.
> Auxiliary accessor preflight follow-up: malformed optional primitive
> attributes, morph-target deltas, `EXT_mesh_gpu_instancing` attributes, and
> skin inverse-bind matrices now report invalid accessor type/count/componentType
> plus missing referenced bufferViews/buffers before import drops or substitutes
> the stream. Bound skin and instancing accessor failures are unsupported because
> best-effort import falls back to static/base primitives; ordinary auxiliary
> stream losses remain approximate.
> Instancing-shape preflight follow-up: `EXT_mesh_gpu_instancing` nodes that
> lack an attributes object, lack any transform accessor, or use invalid
> transform accessor indices now report unsupported compatibility issues before
> import falls back to one base mesh.
...
> later reports.
> Malformed primitive blockers that the importer skips now have the same
> preflight surface: missing `POSITION`, missing/unreadable accessor
> references, missing referenced bufferViews, invalid index accessors, and empty
...
> losslessly project the material-visible high UV set(s) into the two core UV lanes. 2026-06-20 follow-up:
> malformed `KHR_materials_variants` routes now preflight truthfully as material
...
> and rich-material GI. Vertex-level CPU-readable displacement is now an
> approximate shared-BVH geometry feature; explicitly unsupported rows such as
...
> **2026-06-20 material-lobes dzn follow-up:** `pt/material-lobes` and
> `pt/material-lobe-maps` add scalar-lobe and CPU-readable map-backed behavioral
> scenes with clearcoat, sheen, iridescence, anisotropy, and dielectric specular
> panels. They now have committed lavapipe and dzn `dzn-full` golden PNGs plus
> dzn status artifacts proving full-tier boot/render finite non-black with zero
> GPU errors. This closes the scalar and map-backed full-tier material-lobe
> capture tail; specialty-integrator radiometry and broader material A/B
> promotion remain separate proof work.
...
|----|------|-------|-----------|--------|--------|
| **A1** | ✅ **DONE (Campaign 2 Wave A; hybrid-shift wiring 2026-06-16; repaired-scene default A/B 2026-06-21; glossy-research finding pinned 2026-06-22)** | `pt-webgpu/src/index.ts:844-918,1026-1036`; `wgsl/pathTrace/restirPt{Spatial,Compose,Resolve,Temporal,Producer}` | **Implemented/proven for the safe default:** producer→temporal→**spatial (5-neighbour GRIS, no /p_src)**→resolve, then a COMPOSITE megakernel folds the reconnection-indirect into the BEAUTY accumulator via an E0-direct / indirect estimator split (producer-dropped specular pixels fall through to the full path). The formerly harness-only hybrid shift is now live for prefix-1 reuse: reservoirs store a visible-domain replay PDF, temporal/spatial reuse use half-G × source/target BSDF replay-pdf ratio via `restirPtHybridShiftJacobianForPair`, and selected samples refresh the cache for downstream reuse. OFF-path byte-identical. `restirPtReuse:true`+full-tier only. **2026-06-21 repaired-Cornell recapture:** the old 2026-06-11 PASS artifact was stale after the Cornell back-wall repair; first recapture exposed glossy-metal over-brightness, so the default producer now admits only diffuse-safe visible vertices while `restirPtReuseOptions.experimentalGlossyReuse:true` preserves the old glossy/metal research path. Current `results-restir-pt.json` records PASS (`globalRelErr=7.91%`, `varRatio=0.9403`). **2026-06-22 glossy-research recapture:** `results-restir-pt-glossy-research.json` records the opt-in branch as a non-promotable `FINDING` (`globalRelErr=108.42%`, `roiRelErr=297.66%`, `varRatio=7.7140`, `promotion.defaultReady=false`). | Glossy/moderately-specular visible-vertex reuse is now measured and remains an explicit research-mode finding; broader material-furnace/reference radiometry still belongs to V28-B. | done (safe default) / research finding |
...
| **A8** | ✅ **DECIDED (2026-06-10) — biased default retained for realtime; unbiased GRIS documented as first-class opt-in** | `HybridEngineOptions.restirPtReuse`, `temporalGi.wgsl.ts`, `spatialGi.wgsl.ts`, `jacobianShift.wgsl.ts`, `restirPHat.wgsl.ts`, README bias docs, `tools/radiometric-ab/walkaround-ab-results.json` | **Architecture decision:** The default (`restirPtReuse: false`) retains the pre-GRIS Sprint-17 clamped-Jacobian reuse for the realtime frame budget (the unbiased path adds one visibility ray + full-GBH O(K²) MIS cross-evaluation per accepted neighbour — the dominant cost in the GI reuse passes). Remaining documented default-OFF bias sources are B1 Jacobian clamp [0.1,10] (`jacobianShift.wgsl.ts`), B2 no reconnection-visibility ray (OFF variants of `spatialGi`/`temporalGi`), and B3 no full GBH MIS (OFF combine weights). The old B4 centroid-p̂ note is stale: `restirPHat.wgsl.ts` now evaluates `restir_di_compute_phat_xi(lid, xi, surf)`, RIS finalization uses stored `r.xi`, temporal/spatial reuse call the xi-aware helper, and `lo_direct` shades the selected xi. The unbiased GRIS path (`restirPtReuse: true`) is first-class, compile-time gated, fully functional (Phase-1 shift + Phase-2 full-GBH spatial, pairwise-MIS temporal), and the JSDoc specifies exactly when to enable it. H24 follow-up: the default path no longer pays the widened GRIS reservoir memory cost — `restirPtReuse:false` compiles/allocates the compact 20-u32 ReSTIR-GI layout, while the 30-u32 GRIS cache is opt-in only and PPG bakes the matching stride. A compile-time variant-selection pin test added (`__tests__/grisVariantPin.test.ts`), now complemented by `giStructuralGate.test.ts` shader-source stride checks. **2026-06-21 status reconciliation:** `walkaround-ab-results.json` preserves the A8 `NEGLIGIBLE` snapshot (`overall = -0.000020`, about 0.03% of mean, with per-region deltas proof-checked), and the current committed native-Deno radiometric host status now records `PASS-PARTIAL` from a completed full-suite run: A8 is `NEGLIGIBLE`, SUN is `PASS` (receiver ratio = 0.99948), GLASS is `PASS`, and GLOSSY is a material-effect `FINDING`. **2026-06-22 high-SPP native follow-up:** `walkaround-ab-all-spp64.json` recaptures A8/SUN/GLASS/GLOSSY together at 128×128×64 SPP; A8 remains `NEGLIGIBLE`, SUN/GLASS remain `PASS`, and GLOSSY remains `FINDING` with `promotion.defaultReady=false`. The proof checker keeps the do-not-promote warning and requires browser/real-adapter recapture before promotion. The default is NOT being flipped now; this gathers real data for F6 without pretending the broader validation matrix is complete. | Remaining V19 work: browser/real-adapter converged-unbiasedness validation and biased-default error quantification on harder scenes with occlusion/emitter/M-count gradients. The current committed A8 snapshots are evidence for one convex scene, not a default-policy promotion gate. | done (decision); partial A/B evidence / validation ACTIVE |
| **A9** | ✅ **DONE for default/off-default safety (Wave A + v1-closure Wave 3 + 2026-06-15 parity/proof waves + 2026-06-17 A/B finding + 2026-06-19 research-flag guard + 2026-06-22 structured diagnostic) — serial build retained; multi-vertex research open** | `wgsl/bdpt/bdptLightSubpath.wgsl.ts:140-255,575-728`, `wgsl/bdpt/bdptConnection.wgsl.ts:369-443`, `wgsl/pathTrace/kernel.wgsl.ts:900-916`, `pt-webgpu/src/index.ts`, `pt-webgpu/src/__tests__/bdptGlossyLightSubpath.test.ts`, `pt-webgpu/src/__tests__/bdptPlumbing.test.ts`, `tools/radiometric-ab/results-bdpt.json` | **Implemented:** light subpath carries a REAL glossy/specular BSDF (VNDF) at each vertex, bounce cap 3->8, and a 5-row light-vertex record whose row 4 stores hit-local material payload plus a front-face side bit. **Wave 3 fix (1d31f0b):** estimator coherence — extension now samples ONE real-BSDF direction used for BOTH the trace and stored throughput/pdfFwd (was a sampled-then-discarded direction — biased MIS densities); pdfRev patched per PBRT §16.3. **2026-06-15 parity wave:** BDPT light-side material decode now mirrors the shade prologue for mapped base/vertexColor/AO/ORM/transmission/normal/bump/clearcoat-normal/extension/specular/anisotropy plus layer tint/roughness, thin-film reflect tint, Cauchy IOR, and spectral reflectance scalar. **2026-06-15 proof wave:** A9 now has independent numeric TS oracles for row-4 tri/front-face packing, mapped payload transforms, front/back layer selection, thin-film tint mixing, Cauchy IOR, and Jakob-Hanika spectral override; this removes the pure-substring proof residue for that lane. **2026-06-17 proof wave:** the Cornell proof fixture was repaired, secondary emitter-endpoint connection double-count was removed/pinned, and the committed safe-default A/B shows `bdpt:true` endpoint-only mode matches UNI exactly. **2026-06-19/22 guard:** `maxLightBounces>1` is an explicit `experimentalMultiVertex:true` opt-in on pt-webgpu/full-tier and pt-webgl2; the warning now carries `promotionReady:false`, blocker `not-weighted-against-regular-eye-path-strategy`, and the committed result path. **Honest remaining:** serial dispatch (one workgroup; the per-column variant was a spec-undefined cross-workgroup race — documented), a redesigned all-strategy estimator for explicitly opted-in `maxLightBounces>=2`, and independent material-furnace/radiometric A/B before any full-tier/default promotion. | Keep multi-vertex BDPT behind the research warning and explicit flag until the mean mismatch is fixed/proven by an estimator weighted against the regular eye path; default/off-default safe mode is no longer an implementation blocker. | done (safe default) / research tail |
| **A10** | ✅ **Pipeline E2E (Wave A) — package weights not production-default** | `tools/neural-denoiser-training/{train.py,capture-dataset.mjs,export_weights.py,checkpoints/manifest.json}` | **Implemented:** capture→train→export→load CLOSES — `train.py --dry-run/--smoke` exports a valid 535,107-param `.vitrum-model` binary (CANONICAL_PARAM_COUNT pinned to the engine loader; the "vi-neural-weights.json" reference was stale — binary is the real format), round-trip test green; `capture-dataset.mjs` CPU smoke. **2026-06-15 posture fix:** repo research/starter checkpoints are shape/value-validated before allocation, and `neuralWeightsRoundTrip.test.ts` now loads the actual tracked `starter-v1.vitrum-model` and `v2-random.vitrum-model` files through the runtime loader/spec validator. **2026-06-17 truthfulness follow-up:** selecting `denoiser:'neural'` with host weights emits `walkaround-hybrid.neural-host-weights-required`, `capabilities.experimentalFeatures` includes `walkaround-hybrid-neural-denoiser-host-weights`, and `learned-systems-proof-check` pins both claims. **2026-06-20 API resolver follow-up:** `EngineDenoiserMode` now includes `denoiser:'auto'`; walkaround-hybrid resolves it at construction to host-provided `neuralWeights`, then host-provided `oidnModelUrl`, then the preset/default denoiser, and emits `walkaround-hybrid.denoiser-auto-resolved` with `packageProvidesProductionWeights:false`. pt-webgl2/pt-webgpu resolve `auto` to host `oidn.modelUrl` when present or no-denoise when absent, emit backend-scoped `*.denoiser-auto-resolved` warnings, and ledger `auto` as native resolver support. **2026-06-21 checkpoint-classification guard:** `checkpoints/manifest.json` now pins every committed `.vitrum-model` file by role, size, SHA-256, param count, and `productionDefaultEligible:false`; `learned-systems-proof-check` fails on unregistered checkpoints, byte drift, or any production/default checkpoint without a passing `quality-ab-production.json` A/B manifest. **2026-06-23 production-quality guard:** that A/B manifest now has to prove production-scale dataset metadata as well as metrics: at least 500 samples, 1 spp noisy input, >=4096 spp clean references, albedo/normal buffers, capture source, and tonemap. The package still requires host-supplied weights or OIDN assets and does not advertise a production default checkpoint. `neural` remains opt-in/experimental until a validated production checkpoint and quality A/B exist. | Vendor/ship or otherwise bless a production checkpoint; quality A/B. | pipeline done / weights remaining |
...
| **B9** | ✅ **DONE (Wave B) — isotropic/base GGX multiscatter (all 3 backends); pt-webgpu anisotropic approximation improved 2026-06-21** | pt-webgpu `material.wgsl.ts` / `bsdf.wgsl.ts`; pt-webgl2 `glsl/render/get_surface_record_function.glsl.js`; walkaround `ggxBrdf.wgsl.ts` | **Implemented:** Kulla-Conty multiscatter energy compensation in all three base GGX evals (LUT + furnace test on pt-webgpu; furnace-pinned on pt-webgl2, lite-mode skipped). The pt-webgpu anisotropic sampler/eval/PDF are live, and the anisotropic path now uses a conservative projected-roughness Kulla-Conty approximation over the isotropic E LUT instead of treating the lobe stretch as isotropic. This is still deliberately approximate until a native anisotropic furnace/reference model lands. | Anisotropic material-furnace/reference promotion remains proof/research, not a silent contract promise. | done (base GGX + bounded pt-webgpu anisotropic compensation) / anisotropic promotion tail |
...
- **D13 — walkaround/RC `ptr<storage>` traversal → naga/Firefox portability (TRACKED PROGRAM, ADDED 2026-07-20 — D4 decision Option B)** ◻ SCOPED / NOT SCHEDULED. The walkaround-hybrid + `@vitrum/walkaround-rc` production shaders pass storage pointers by function parameter (`ptr<storage>` — the shared-bvh TLAS traversal helpers), which requires the WGSL `unrestricted_pointer_parameters` capability. Tint/Chrome accept it, but **naga (Firefox) and Deno's wgpu-native REJECT** it and there is no WebGPU `enable` path → the runtime walkaround shaders are **Chromium-only**. **Decision (D4 Option B, 2026-07-20):** keep `ptr<storage>` shipping; make the Chromium-only reality honest, tracked, and measured rather than refactoring the traversal core now. Delivered: (1) a non-fatal verbatim-compile tracked metric in `tools/shader-gate/gate.mjs` that also attempts the un-patched shipped source and reports the naga-incompatibility count (`[shader-gate] verbatim-compile metric … N/M shipped shaders reject on naga verbatim`); (2) Chromium-only documentation in `packages/walkaround-hybrid/README.md` + `plan/renderer-fidelity-matrix.md`. **Remaining (this row):** the actual traversal-core refactor to drop `ptr<storage>` (pass indices/values instead of storage pointers, or inline the traversal) so Firefox/naga can compile the shipped shaders. This is a **HIGH-risk, T1-smoke-gated** program (the CPU brute-force TLAS/BVH oracles + naga-compile gate must stay green through the rewrite); it is intentionally **not planned here**. **Effort: L (traversal rewrite + full oracle re-validation).**
...
- **MaterialSpec consumption matrix** (items H46–H52): the original "~60 fields
  ...
  stacks; scattering and front/back absorption layers are approximate), and
...
- **A8 DECIDED (2026-06-10) + A/B ELEVATED (2026-06-13):** biased default retained for the realtime budget; bias quantified+documented; unbiased GRIS first-class opt-in. The default-flip question (F6) is NOT closed — it is now gated on the `VQ-WALKAROUND-RADIOMETRIC-AB` partial-proof validation row (converged-unbiasedness of GRIS-on + measured error of the biased default on the wsl-gpu/browser-real-adapter rigs). Revisit the flip with those numbers.
...
| Target | Done when |
|--------|-----------|
| **glTF** | `loadGltfAsset` → `loadGltfForEngine` handles URL/GLB/JSON+external resources; `analyzeGltfAsset` + `rankGltfBackends` are complete; `GltfSceneController` drives skin/morph/TRS; every extension in `REQUIRED_EXTENSION_SUPPORT` (`featureReport.ts:436-461`) has import + compatibility + test; zero silent `console.warn` in adapter (string warnings in return values, plus structured import diagnostics for converter-owned degradations). |
...
**Explicit non-goals** (otherwise "100%" is undefined): displacement tessellation, production neural weights, SPPM default-promotion/radiometric-oracle proof beyond the shipped A4-progressive implementation, cross-host GPU certification. glTF point/line topology is no longer a non-goal: it imports as explicit fallback-generated mesh geometry.
...
| **GATE-02** | ✅ CLOSED — `core/src/__tests__/materialNativeEvidence.test.ts` enumerates every `native` material row from `BACKEND_PROMISE_LEDGER`, and `core/src/__tests__/materialApproximateEvidence.test.ts` now enumerates every `approximate` material row across walkaround-hybrid, pt-webgl2, and pt-webgpu. Both gates fail unless each backend/field has named packer/shader/classifier/readback or truthfulness-surface evidence with existing test/source file paths. | This is the ledger-evidence gate; renderer A/B and material-furnace proof still live in Phase 5 where required. Approximate rows remain approximate by contract; the new gate prevents un-evidenced or stale approximate promises. |
...
2026-06-20 sampler-policy truthfulness follow-up: core/glTF `TextureRef.magFilter` / `minFilter` / `mipFilter` now reach runtime diagnostics instead of disappearing after compatibility analysis. pt-webgpu appends per-map mip-policy and mag/min filter-policy lanes to the full-tier material texture descriptor; the WGSL explicit-LOD sampler honors authored `mipFilter:"none"`, nearest mip selection, linear mip interpolation, and nearest-vs-linear mag/min policy for regular material maps through a `textureLoad` nearest branch plus the existing filtered `textureSampleLevel` branch. The pt-webgpu adjoint replay pass consumes those same descriptor lanes for replayed material maps. 2026-06-20 pt-webgl2 closure: the material texture layout now stores one sampler-policy texel per map (`wrapS`, `wrapT`, mip policy, packed mag/min filters), the RGBA32F texture array uploads a complete CPU-generated mip chain, and `material_struct.glsl.js` resolves nearest/linear filtering plus no/nearest/linear mip selection manually with `texelFetch`, so authored sampler policies are no longer runtime/classification approximations on pt-webgl2. Same-day walkaround closure: atlas metadata now packs mag/min/mip bits beside wrap/UV selection, WGSL samples nearest or bilinear base-level texels manually, and compatibility/runtime warnings are narrowed to policies that require mip LOD or min/mag footprint selection. Runtime/static diagnostics now warn only for renderer paths that still truly approximate sampler policy, including pt-webgpu bump-map raw finite-difference sampling and walkaround LOD/footprint-dependent atlas policies. Remaining pt-webgpu/walkaround sampler-parity proof belongs to renderer A/B validation, not hidden API/data loss.
...
| Task | Code | Footgun |
|------|------|---------|
| ~~Source paths on every issue~~ ✅ DONE | `featureReport.ts`, `gltfAssetApi.test.ts` | `GltfCompatibilityIssue.path` is now required, analyzer source-path maps cover extensions/primitives/materials/scene cameras, and the test asserts every issue has a non-empty path including `materials[0].normalTexture`-style material paths. 2026-06-20 follow-up: malformed material texture references now preflight as `textureReferenceIssues` with exact paths for missing texture indices, missing selected `texture.source`, missing images, source-less images, missing image bufferViews, and image bufferViews whose `buffer` record is missing, matching best-effort import dropping the affected map instead of letting planner compatibility look clean. Same-day primitive follow-up: `malformedPrimitives` now also catches POSITION/index accessors whose bufferView exists but references a missing `buffers[n]` record, matching import's unreadable-position/unreadable-indices skip paths. Same-day animation follow-up: `malformedChannels` now catches animation sampler input/output accessors with invalid component types, missing referenced bufferViews, or bufferViews whose `buffer` record is missing, matching import's `unreadable-animation-sampler` skip path. Same-day material-reference follow-up: base `primitive.material` indices outside `materials[]` now preflight as `primitive.material.missing-material=approximate` at `meshes[n].primitives[m].material`, and import emits `material-not-found` so `reject-degraded` rejects instead of silently substituting the default material. Same-day loader-error follow-up: typed external buffer/image resource errors now carry `sourcePath` (`buffers[n].uri` / `images[n].uri`) beside URL/kind/status/reason, and `GltfCompatibilityError` preserves structured `failureDetails` for compatibility issues, import diagnostics, and texture-readiness failures while keeping legacy formatted `failures`. |
...
| ~~Spec-gloss glossiness-alpha~~ ✅ DONE (explicit pre-decode downgrade + decoded roughness bake) | `materials.ts` imports `specularGlossinessTexture` RGB as `specularColorMap`; `featureReport.ts` emits `KHR_materials_pbrSpecularGlossiness.specularGlossinessTexture.glossinessAlpha=approximate` with source path for planner/strict modes; `decodeSceneTextures()` / `loadGltfAndDecodeTextures()` now bake alpha glossiness into a generated linear `roughnessMap` for both `cpu-linear` and `webgpu` texture targets; `loadGltfForEngine(..., compatibilityMode:'reject-degraded')` no longer keeps this specific alpha issue active after the bake succeeds for every authored spec-gloss texture path, and best-effort bridge tests verify the engine receives the decoded scene with the generated `roughnessMap`. 2026-06-20 engine-subpath follow-up: `@vitrum/engine/gltf` runtime profile revalidation now mirrors that source-path-scoped satisfaction check, requiring a matching `roughnessMap` decode entry with `handleColorSpace:'linear'` instead of accepting any unrelated decoded roughness map. Tests: `gltfExtensionPolicy.test.ts`, `gltfAssetApi.test.ts`, `gltfStrictPtWebgpuTier.test.ts`. | No silent roughness-map lie: scalar `glossinessFactor` drives roughness before decode; host-supplied pixel decode closes the per-pixel legacy glossiness path only when all source paths bake. The broader archived spec-gloss model conversion remains approximate unless callers accept it. |
...
| ~~Morph `TEXCOORD_0/1` contract policy~~ ✅ DONE (high-UV remap follow-up 2026-06-19) | `SkinnedMeshPrimitive.morphTargetUvs` / `.morphTargetUv1s`, `skinSolver.ts`, `gltfToScene.ts`, `featureReport.ts`, `sceneController.ts`, `gltfModesMorphsAnimations.test.ts`, `gltfAssetApi.test.ts`, `skinnedMeshIngestion.test.ts`, `pt-webgl2/src/scene/solveSkinPrimitives.ts`, `skinSolve.test.ts`, `mutationMatrix.test.ts` | glTF morph-target UV deltas for the two UV lanes the core Scene contract actually carries now import, preflight as supported when the matching base UV stream exists, blend through the shared CPU skin solver, propagate through animation patches, and reach pt-webgpu/pt-webgl2/walkaround attribute uploads. The 2026-06-19 follow-up made this source-truthful for high static UV remaps; the 2026-06-25 follow-up generalizes the importer to project up to two material-visible UV sets into core `uvs`/`uv1`. Morph deltas for whichever glTF `TEXCOORD_N` semantic is assigned to a core UV lane feed the matching `.morphTargetUvs` / `.morphTargetUv1s` stream, and stale deltas for unassigned UV semantics remain ignored. Remaining unsupported cases are explicit: missing base UV streams, material routing that needs more than two UV lanes, or high morph UV lanes that are not assigned to core `uvs`/`uv1` still emit `ignored-morph-target-texcoord` diagnostics plus `morphTargetTexcoords=unsupported` compatibility issues. |
| ~~Cameras~~ ✅ DONE (2026-06-16 strict-mode alignment; 2026-06-19 metadata + live-controller follow-up) | `featureReport.ts`, `cameraMetadata.ts`, `gltfToScene.ts`, `sceneController.ts`, `engineBridge.ts`, `gltfAdapter.test.ts`, `gltfAssetApi.test.ts`, `sceneController.test.ts`, `gltfProgressiveSubpath.test.ts` | `sceneGraph.cameraPaths` records `cameras[n]`; compatibility now emits `scene:cameras=approximate` to match the importer’s `ignored-camera` diagnostic. `gltfToScene()` also returns selected-scene `cameras[]` metadata, one entry per reachable camera node, including source paths, camera/node names, perspective/orthographic projection scalars, and the authored node world matrix. `loadGltfAsset()` and `loadGltfForEngine()` preserve that metadata through `asset.cameras`. `GltfSceneController.cameras` now stays live: controller construction backfills metadata when callers omit `result.cameras`, `applyAnimation()` / `blend()` recompute camera world matrices from the same sampled node hierarchy used for primitives/skins/instancing, and `resetPose()` restores rest camera metadata. `reject-unsupported` accepts otherwise renderable assets with glTF cameras because Vitrum render cameras are still supplied through `FrameInput`; `reject-degraded` rejects before engine construction when callers require zero approximate scene features. |
...
| ~~Sparse accessors~~ ✅ DONE | `accessors.ts`, `accessors.test.ts`, `gltfAdapter.test.ts`, `gltfModesMorphsAnimations.test.ts`, `gltfAssetApi.test.ts`, `featureReport.ts` | Sparse patches now have focused coverage for base+pure-sparse accessors, unsigned-byte sparse indices, byte offsets, strided base data, normalized values, integer index accessors, sparse `JOINTS_0` skin streams, and out-of-range/invalid sparse-index diagnostics. Compatibility preflight now reports hard POSITION/index sparse storage blockers plus degraded optional sparse patch losses on auxiliary primitive attributes, morph deltas, instancing attributes, skin inverse-bind matrices, and animation sampler streams. |
...
| ~~`KHR_materials_variants` at runtime~~ ✅ DONE | `GltfSceneController.setVariant(name/index/undefined)` uses importer-emitted `materialVariantBindings` + `convertedMaterials` to patch only affected primitive materials; `loadGltfForEngine()` now forwards that metadata into bridge-created controllers; decoded loads now decode the inactive material table before controller creation; `updatePrimitive` fast path falls back to `setScene(nextScene)` on rejection; `resetPose()` preserves the active material variant. 2026-06-20 follow-up: `analyzeGltfAsset()` now reports malformed variant mappings that reference missing material indices, missing/malformed variant lists, or missing root variant indices as exact source-pathed `support:"unsupported"` material compatibility issues, matching the runtime base-material fallback if such a variant is selected. Tests: `sceneController.test.ts` runtime switch + fallback, `gltfAssetApi.test.ts` bridge-created controller switch + decoded inactive variant texture, `gltfExtensionPolicy.test.ts` import-time selection and malformed-mapping preflight. | Variant switch now invalidates material fast-path caches by issuing material patches through the same engine patch channel as animation updates, without reintroducing raw texture handles after a decoded load. |
...
| ~~`anisotropy` / `anisotropyRotation` / `anisotropyMap`~~ ✅ DONE (2026-06-13; dispersion/tangent-frame follow-ups 2026-06-20) | Scalar strength/rotation pack into reserved lanes `s11.a` / `s17.b`; `anisotropyMap` packs into `s6.b`, transform texels `95..96`, UV bit 19, and the final wrap-mode pair. GLSL decodes the map and samples RG/B according to KHR_materials_anisotropy (`B` strength, `RG` rotation offset), feeding anisotropic GGX sampling/eval/PDF in `bsdf_functions.glsl.js`. 2026-06-20 follow-ups: pt-webgl2 Cauchy/hero-wavelength dispersion transmission now uses the same `ggxDirectionForSurface()` visible-normal helper as ordinary transmission, so anisotropic glass does not become isotropic only when dispersion is active; the WebGL2 surface record now builds the BSDF `normalBasis` from the interpolated authored/generated tangent after Gram-Schmidt projection against the final shaded normal, falling back to the normal-only frame for missing/degenerate tangents, so anisotropy-map RG rotation is tangent-frame relative instead of arbitrary-normal-frame relative; ordinary and Cauchy transmission no longer apply the removed post-refraction `rand2(47)` GGX perturbation, leaving the sampled microfacet half-vector as the sole rough-refraction sample represented by `transmissionEval()`'s PDF; Cauchy transmission now uses the same hero-wavelength eta for dispersion sampling, transmission half-vector reconstruction, `transmissionEval()`'s denominator, and lobe-selection Fresnel weights instead of sampling with hero eta while evaluating with base `surf.eta`. Tests: `materialsTexture.test.ts`, `materialStrideParity.test.ts`, `engineContract.test.ts`, `bsdfLobeSelectionPolicy.test.ts`, core ledger contract. |
...
| Adjoint | `adjointPass.wgsl.ts`, `pathTraceAdjoint.wgsl.ts`, `inverseSession.ts` | ◻/✅ Scoped path replay now consumes `req.samples`, replays the frozen inverse sample sequence, and covers opaque direct-light/unlit-primary fields including baseColor, roughness, metallic, `aoMapIntensity`, camera-visible `lightMapIntensity`, `envMapIntensity` for direct HDRI/procedural-sky NEE, emissive/emissiveIntensity, specular, clearcoat, `clearcoatNormalScale`, sheen, iridescence, `iridescenceThicknessRange` (map-free and readable thickness-map texel chained), anisotropy, their documented local map factors, deterministic direct emitter `color`/`intensity`, and delta/soft-sun directional plus point/spot/stochastic area-measure rect/disc/mesh-area direct lights. 2026-06-16 soft-sun follow-up mirrors the forward directional cone sample in `adjointPass.wgsl.ts` and keeps compatible soft directional material/emitter targets on path replay. 2026-06-18 environment-NEE follow-up adds the same HDRI/procedural-sky CDF replay shape used by the forward direct-light branch (bindings 20/21, texel `.w` solid-angle PDF, rotationY convention preserved), scales that replay by the hit material's descriptor `[4].w` `envMapIntensity`, and exposes a scoped path-replay gradient for that direct environment-lighting scale. 2026-06-19 normal/bump/clearcoat-normal follow-up: top-level opaque `normalMap`, `bumpMap`, and `clearcoatNormalMap` are replayed through the existing linear texture-array descriptors and UV transforms, prefer the forward shader's authored/generated tangent.xyzw handedness before UV-gradient fallback, direct-light BRDF/emitter gradients evaluate at the perturbed shading normal, clearcoat scalar gradients evaluate against the independent clearcoat lobe normal, and `normalScale` / `bumpScale` / `clearcoatNormalScale` now have scoped replay-local gradients for frozen direct lighting. Current follow-up: `adjointPass.wgsl.ts` now computes replay-local central differences of the full normal stack, so `normalScale` stays on path replay through bump/clearcoat-normal tangent frames and `bumpScale` stays on path replay through clearcoat-normal tangent frames instead of downgrading those nested-map cases to render-level finite difference. 2026-06-20 sampler-replay follow-up: replayed material maps now use the same authored mip/filter descriptor lanes as the forward full-tier sampler for base/emissive, roughness/metallic, AO/light, normal/bump/clearcoat-normal, clearcoat/sheen/iridescence/specular/anisotropy maps; this closes the previous replay-vs-forward sampler-policy skew for already replayed map factors. Finite-difference inverse sessions still admit transport/visibility/geometry controls (`transmission`, `thickness`, `attenuationColor`, `attenuationDistance`, `dispersionAbbeNumber`, `scatteringCoefficient`, `scatteringAnisotropy`, `scatteringCoefficientRGB`, `opacity`, `alphaCutoff`, `displacementScale`, `displacementBias`) with structured path-replay downgrades. 2026-06-17 texel-emission truthfulness follow-up: explicit `mesh-area` emitter `color`/`intensity` targets with readable `emissiveMap` stay on path replay when the authored color/intensity denominators are nonzero, because the adjoint chains through packed per-triangle radiance. 2026-06-19 source-factor follow-up: the mesh-area packer now emits a `meshAreaLightSourceFactors` side buffer and the adjoint pass differentiates `color * intensity * sourceFactor`, so zero authored color channels no longer force a finite-difference downgrade. Current follow-up: mesh-area emitter inverse fits bind an adjoint-only replay stream that preserves zero-power explicit mesh triangles, so zero-intensity mapped emitter `intensity` targets stay on path replay without changing forward render packing; capped/reordered explicit mesh-area emitter `color`/`intensity` targets now stay on path replay too because the source-factor `.w` lane carries a stable owner slot independent of packed triangle order. Non-color/intensity emitter targets still stay on `path-replay-unsupported-emitter`. 2026-06-18 classification follow-up: scene/material blockers, including receiver-material blockers for emitter-parameter fits, now propagate the same reason-specific downgrade codes (`path-replay-unsupported-visibility`, `path-replay-unsupported-transport`, `path-replay-unsupported-normal`) for alpha coverage, transmission/volume/spectral/layered, and unsupported normal/visibility domains instead of collapsing those known tails into a generic material/receiver warning. Same-day geometry diagnostic follow-up: displacement-map replay blockers now use contract-level `path-replay-unsupported-geometry` with `finiteDifferenceReason:"geometry"` instead of the generic material code, so hosts can separate geometry/BVH displacement limits from BRDF material limits; same-day scalar follow-up admits `displacementScale`/`displacementBias` as finite-difference inverse controls with the same geometry diagnostic, without promoting native displacement rendering or path replay. 2026-06-18 estimator-regime follow-up: the real engine now reports active BDPT, ReSTIR-PT reuse, and non-`none` caustic strategies to inverse sessions, and requested path replay downgrades with `path-replay-unsupported-render-regime` before using an adjoint that does not mirror those render contributions. 2026-06-19 inverse direct-light-sum follow-up: `inversePausedRender.test.ts` now opens a real `createPTEngine_WebGPU()` inverse session on a point-light + HDRI scene, captures the packed frame params, and asserts ordinary renders keep `directLightingMode=0` while inverse readbacks use `directLightingMode=1` / `directLighting:"summed-expectation"`; the full/lite kernels sum direct-light candidates in that scoped replay baseline, so real multi-light inverse sessions no longer downgrade for selection alone. 2026-06-20 area/env MIS follow-up: summed-expectation rect/disc, mesh-area, and environment NEE replay now computes the same per-light BRDF/light power-heuristic factor as the forward direct-light estimator; material and emitter gradients are scaled by that MIS weight, and normal-stack central differences recompute it for perturbed normals. Still OPEN for full-path parity: alpha-map/alpha-visibility adjoints, transmission/thickness/attenuation-color/attenuation-distance/dispersion/scattering/displacement, normal texture-pixel/UV gradients, environment BSDF-escape/indirect transport, layered/volume/spectral material domains, exact texel-PDF mesh emission promotion for emitter-parameter adjoints, indirect paths, GPU inverse-fit recaptures, derivative-through-MIS proof for every material scalar, and full material-lobe A/B proofs. |
| Present | `present.wgsl.ts` tonemap only — no BSDF | N/A |

Adjoint dormant-map note (2026-06-20, narrowed 2026-06-21): the remaining alpha-map tail above means active mask/blend visibility. Opaque-material `alphaMap` is now code-closed for path replay because forward `alphaTestPassThrough` returns before sampling alpha when `alphaMode:"opaque"`. Same-day scalar-alpha follow-up: map-free `alphaMode:"mask"` stays on path replay when scalar `opacity >= alphaCutoff`, and map-free `alphaMode:"blend"` stays on path replay when scalar `opacity >= 1`, provided the primitive has no non-opaque `COLOR_0.a`; readable all-opaque base-color alpha / standalone alpha-map coverage is now also treated as inert, while fractional coverage still emits `path-replay-unsupported-visibility`. Transmission/thickness maps plus spectral/dispersion/scattering metadata with scalar `transmission` at zero are likewise code-closed for opaque direct-light path replay; positive scalar `transmission` is now also treated as dormant when a readable `transmissionMap` has zero R coverage everywhere, so sidecar `thicknessMap`, spectral/dispersion, and scattering metadata no longer force a fake transport downgrade in that effective-zero case. Identity front/back layers, layer-normal-scale metadata without a layer normal map, plus empty thin-film stacks no longer force a downgrade because they pack to the same BRDF inputs as absent layers. Active transmissive/spectral/volume/layered/thin-film transport still downgrades. `inverseSession.test.ts` pins dormant-alpha/dormant-transport/dormant-layer replay plus active-alpha/active-transport finite-difference downgrade.

Adjoint attenuation-distance seed note (2026-06-20): finite-difference inverse sessions no longer silently materialize missing `attenuationDistance` as `1` when the renderer would treat undefined/`Infinity` as no finite absorbing medium. `materials.<id>.attenuationDistance` now requires either a finite positive authored scene value or an explicit finite `parameter.initial` seed; `inverseSession.test.ts` pins missing, `Infinity`, and zero scene seeds as constructor errors plus the explicit-initial escape hatch. This keeps the transport row honest without claiming analytic transport adjoints.

Adjoint environment-candidate note (2026-06-20): path-replay light-selection diagnostics now mirror forward HDRI packing for unreadable and zero-luminance maps. `inverseSession.ts` uses the shared `readEnvironmentMapPixels()` normalization plus the same luminance × sin(theta) positivity test as `environmentPacking.ts`, so opaque/unreadable HDRI handles and all-black readable HDRIs no longer count as direct-light candidates and no longer force a fake `path-replay-unsupported-light-selection` downgrade when an actual point/spot/area emitter is the only radiance source. Positive HDRIs and procedural skies with positive intensity still count as real candidates; only engines or test hooks that report `directLighting:"sampled-selection"` keep the existing downgrade. Same-day runtime-environment guard: if a plain-JS/extension scene carries a contributing environment kind outside the pt-webgpu replay domain (`none`, `hdri`, `procedural-sky`), requested path replay now downgrades with `path-replay-unsupported-environment` instead of silently treating that unknown source as replay-compatible. The real pt-webgpu inverse baseline now reports summed direct lighting and avoids that downgrade while ordinary renders keep sampled selection.

Adjoint emitter-candidate note (2026-06-20): path-replay light-selection diagnostics now mirror the emitter packer for authored-but-non-contributing area lights. `directLightCandidateLabels()` filters zero-radius / degenerate-normal `disc-area` emitters the same way `packDiscAsRect()` skips them, and it uses `meshAreaEmitterAdjointRangeForScene()` so missing, analytic, empty, zero-instance, invalid-index, or fully degenerate explicit mesh-area emitters do not create fake multi-light downgrades. Valid mesh-area emitters with replayable triangles still count as direct-light candidates. They downgrade only when the forward context still reports `directLighting:"sampled-selection"`; the real pt-webgpu inverse context now reports `directLighting:"summed-expectation"` and stays on path replay for selection-compatible targets.

Adjoint emitter/environment/normal-stack receiver note (2026-06-20): emitter `color` / `intensity`, material `envMapIntensity`, and normal-stack scalar targets (`normalScale`, `bumpScale`, `clearcoatNormalScale`) now include iridescent receiver BRDF values instead of failing closed for any positive `iridescence`. `pathTraceAdjoint.wgsl.ts` exposes an iridescence-aware anisotropic BRDF value helper and `adjointPass.wgsl.ts` passes the effective scalar/map iridescence and thickness state into emitter radiance-gradient scattering, environment-intensity gradients, and replay-local normal-stack finite differences; `inverseSession.ts` therefore allows iridescent receivers for those targets while keeping alpha/transmission/layered/spectral/volume/displacement receiver domains on structured finite-difference downgrades. `inverseSession.test.ts` pins mapped-iridescence receiver replay for `emitters.*.intensity`, `materials.*.envMapIntensity`, and the normal-stack scalar targets.

Procedural-sky inverse replay note (2026-06-20): the adjoint pass intentionally samples environment lighting only through the environment-map texel/CDF buffers. Positive `procedural-sky` scenes still stay in the scoped path-replay domain because `environmentPacking.ts` bakes Preetham sky into that same equirect/CDF path before upload; `inverseSession.test.ts` now pins positive procedural-sky `envMapIntensity` path replay and keeps unsupported-environment diagnostics reserved for non-baked runtime environment kinds.

Additive-lobe inverse classifier note (2026-06-20): fixed scalar iridescence on an otherwise replay-compatible material no longer forces map-free `clearcoat`, `clearcoatRoughness`, `sheen`, `sheenColor`, or `sheenRoughness` fits to finite difference; those fields are additive-lobe adjoint lanes and remain on scoped path replay. Base-BRDF scalar fits such as `roughness` still downgrade when active iridescence is present, because their replay derivative is coupled to the thin-film state. Unsupported emitter kinds now only poison path replay when they have positive intensity/color and therefore can contribute direct light; zero-intensity or black extension emitters are ignored, while contributing unknown kinds still emit `path-replay-unsupported-lighting`.

Alpha-visibility inverse classifier note (2026-06-20, narrowed 2026-06-21): `inverseSession.ts` now mirrors pt-webgpu material-texture upload semantics before declaring texture alpha a visibility blocker. Readable RGB/one/two/three-channel `baseColorMap` payloads expand to alpha 1, readable RGBA base-color textures whose alpha is fully opaque stay replay-compatible, readable standalone `alphaMap` payloads whose R coverage is fully opaque stay replay-compatible, and map-free `mask` alpha scalars that are stably above `alphaCutoff` are treated as zero-gradient replay lanes. Readable RGBA payloads with fractional alpha, standalone `alphaMap` coverage below 1, vertex `COLOR_0.a`, mask coverage near/below cutoff, and fractional blend opacity still produce `path-replay-unsupported-visibility` until replay mirrors those discontinuous visibility terms.

Primary-hit inverse classifier note (2026-06-20): camera-visible additive/material-output fields now stay on path replay when unrelated transport/layer/spectral/volume metadata is present but not part of that primary-hit derivative. `emissive`, `emissiveIntensity`, `lightMapIntensity`, unlit `baseColor`, and unlit `aoMapIntensity` no longer downgrade solely because a material also carries transmission/thickness/layer/scattering metadata. Alpha visibility, displacement maps, and opaque `MaterialSpec.extensions` still fail closed with structured visibility/geometry/material diagnostics.
...
| ~~Caustics~~ ✅ DOC/TEST CLOSED | pt-webgl2 `manifold-nee` remains a deterministic refraction-walk heuristic, not Newton-solve MNEE (`options.ts`, README, core factory note). `renderer-fidelity-matrix.md` grades pt-webgl2 caustics `approximate`, and `engineContract.test.ts` pins that the backend surfaces caustic choices through `capabilities.causticStrategy` without advertising native MNEE feature support. 2026-06-20 follow-up: constructing pt-webgl2 with `causticStrategy:"manifold-nee"` or `"photon-map"` now emits `pt-webgl2.caustic-strategy-approximation` with the exact approximation class, so the runtime cannot be mistaken for promoted MNEE/photon mapping. | Keep pt-webgpu `manifold-nee` as the validated reference; pt-webgl2 caustics stay approximate unless a real MNEE port lands. |
...
Already native. **glTF instancing:** ordinary glTF multi-node reuse still flattens to separate primitives. **`EXT_mesh_gpu_instancing` import + node animation DONE (2026-06-16; skinned/morphed fallback-expanded 2026-06-20):** accessor-driven node instancing now imports to core `InstancedMeshPrimitive`, with `nodeWorld * instanceTRS` baked into each matrix; required use is accepted; feature reporting predicts `instanced-mesh` instead of an unsupported extension; direct and `loadGltfForEngine()` bridge-created `GltfSceneController` instances carry the local instance matrices and patch `instances[]` when the instanced node or an ancestor animates. Malformed accessors and custom per-instance attributes remain explicit fallback edges: malformed or no-transform data imports the base mesh once with an `ignored-gpu-instancing` diagnostic, and custom non-transform attributes warn and are ignored. Skinned/morphed instancing is now renderable via fallback expansion: `gltfToScene()` creates one skinned/morphed primitive per authored instance transform, reports `fallback-expanded-gpu-instancing`, preserves material/variant/animation provenance for each generated primitive id, and the controller patches each expanded transform when the instanced node or an ancestor animates. Compatibility reports this as `EXT_mesh_gpu_instancing.skinnedOrMorphed=fallback-generated-mesh`; `reject-unsupported` accepts it, while `reject-degraded` rejects it for callers that require native instanced skinning.
...
> (spectral/dispersion/thin-film/layers); scattering and displacement are handled
...
| Emitter `castShadow` | DDGI/ReSTIR/RC direct-light paths | ✅ CODE CLOSED / ledger native: analytic point/spot payload packs binding-13 lane `[13]`; shared `EmitterTri` packs lane `[19]`; ReSTIR-DI candidate visibility + shade visibility gate on `e.castShadowDisabled`; DDGI/RC area-emitter NEE skip the emitter shadow ray; DDGI and RC fixture/sun lights carry castShadow-disabled flags; main direct sun gates its visibility ray from the scene directional emitter flag. 2026-06-21 follow-up: material-backed ReSTIR-DI mesh emitters now preserve both source primitive `castShadow:false` and explicit `mesh-area` emitter `castShadow:false`, including merged/TLAS emissive-map texel-cell subemitters. | Regressions: `directLightEmitterCore`, `probeUpdateLights`, `rcLightsLayoutPin`, `emitterCastShadowWgsl`, `rcLightEvalWgsl`, `hybridEngineFrameOrchestrator`, and `engineContract`. |
...
|-----------|---------|-------|
| Atlas build | ✅ FOURTH SLICE + 2026-06-18 truthfulness follow-up: `pipeline/materialTextureAtlas.ts` | `baseColorMap`, `emissiveMap`, specular-color, and sheen-color raw/DataTexture handles are inverse-sRGB decoded into linear RGBA32F array layers; `normalMap`, `bumpMap`, `roughnessMap`, `metallicMap`, `aoMap`, `alphaMap`, `transmissionMap`, `thicknessMap`, `lightMap`, specular-intensity, clearcoat, clearcoat-roughness, clearcoat-normal, sheen-roughness, anisotropy, iridescence, and iridescence-thickness maps are packed as linear map layers and sampled from their glTF channels. 2026-06-20 scalar-map semantics follow-up: walkaround shade/ReSTIR-DI material payloads and DDGI probe material hits now apply `roughness * roughnessMap.g` and `metallic * metallicMap.b` instead of letting those map channels replace the scalar factors. Scalar `envMapIntensity` is packed in atlas metadata and consumed by HDRI ReSTIR-DI scoring/reuse/resolve. 2026-06-21 face-layer follow-up: `frontLayer` and `backLayer` pack face-selected transmission and roughness controls into atlas metadata, and shade/OIT/ReSTIR-DI/ReSTIR-GI/DDGI paths apply the transmission/roughness approximation. 2026-06-21 volume-scattering follow-up: scalar/RGB `scatteringCoefficient*` plus `scatteringAnisotropy` pack into atlas metadata and feed bounded single-scatter tint/softening in shade, OIT, ReSTIR-DI, ReSTIR-GI, and DDGI; this is approximate realtime volume response, not delta-tracked volumetric transport. Unreadable CPU texture handles emit `walkaround-hybrid.unreadable-material-texture-map` through `onWarning` at scene load/material rebuild with `fallback:"map ignored"` and, when the `TextureRef` came from the glTF adapter, source metadata (`sourcePath`, texture/image/sampler indices, URI, MIME type, and source extension) so hosts can locate the exact incompatible asset input. Atlas-backed maps with unsupported `TextureRef.texCoord` values now emit `walkaround-hybrid.unsupported-material-texture-texcoord` and are explicitly ignored; only UV sets 0 and 1 are supported. Displacement is handled by shared-BVH vertex displacement when CPU-readable; spectral/dispersion/thin-film/full-layer-stack families stay explicit unsupported warnings, not silent atlas drops. |
...
Document in ledger + planner: `spectralAttenuation`, `dispersionAbbeNumber`, and `thinFilmStack` are permanent `unsupported` rows on walkaround. These fields are now pinned by walkaround unit + engine-warning tests on both `setScene()` and `updatePrimitive()`, including the `KHR_materials_dispersion` source path in glTF compatibility reporting. `scatteringCoefficient`, `scatteringAnisotropy`, and `scatteringCoefficientRGB` are no longer in this bucket: they are approximate realtime volume-scattering rows, with atlas-packed sigmaS/aniso controls consumed in shade, OIT, ReSTIR-DI, ReSTIR-GI, and DDGI; true volumetric random walks/delta tracking remain outside walkaround scope. `frontLayer` and `backLayer` are no longer in this bucket: they are approximate face absorption layers, with transmission plus roughness plus layer-local normal maps consumed in shade, OIT, ReSTIR-DI, ReSTIR-GI, DDGI, and RC; full layered/TMM transport remains unsupported. `displacementMap` / `displacementScale` / `displacementBias` are not in this unsupported bucket: they are approximate shared-BVH displacement when CPU-readable: vertex-only by default, with bounded uniform CPU microdisplacement when `displacementSubdivisions` is set. **Arbitrary glTF 100%** routes assets using unsupported specialty fields to pt-webgpu via `rankGltfBackends` — walkaround 100% ≠ all fields native.
...
| H34 BVH degenerates | `buildArrayBvh.ts`, `worldSpaceMerge.ts`, `tlas.ts`; `nanTriangleFilter.test.ts`, `oobIndexFilter.test.ts`, `worldSpaceMergeInvalidTriangles.test.ts`, `tlas.test.ts` | ✅ CODE CLOSED: BLAS already filters non-finite and out-of-range-index triangles with warnings and returns the empty-BVH shape when every triangle is invalid; this wave added TLAS parity by filtering non-finite build instances (AABB or `worldToLocal`) with warnings, throwing clearly when no finite instance remains, and rejecting non-finite/inverted refit AABBs before mutating node bounds. 2026-06-20 follow-up: `mergeWorldSpaceFromCore()` now filters out-of-range and non-finite triangles before they enter the shared world-space merged stream, so pt-webgl2, walkaround merged-BVH, and pt-webgpu merged-mode consumers no longer inherit stale pre-filter `triangleCount`, `mergedIndices`, `mergedTriMaterialId`, or `meshVertexRanges` after malformed triangles are rejected. |
...
✅ **WALKAROUND RESOLVER RAW-HDRI FOLLOW-UP (2026-06-21):** `walkaround-hybrid`'s `resolveEnvironmentMap` extension callback can now return a CPU-readable `rawHdri` equirect payload, not just scalar sky averages. The resolver-provided payload runs through the same directional IBL map/CDF builder as native raw HDRI handles, while optional resolver `skyTint` / `skyIrradiance` values remain scalar fallback overrides.
...
| ~~Multiple UV sets~~ ✅ CODE/TEST NARROWED | `TEXCOORD_1` imports/consumes as `uv1`; generated tangent frames use `TEXCOORD_1` when tangent-space maps select `texCoord:1`. 2026-06-16 follow-up: a single higher glTF UV set (`TEXCOORD_N`, `N > 1`) can still be losslessly loaded into core `uv1` and its primitive-local `TextureRef.texCoord` values rewritten to `1`. 2026-06-25 follow-up: `gltfToScene()` and the compatibility analyzer now project up to two material-visible UV sets into core `uvs`/`uv1` per primitive, so combinations such as `TEXCOORD_1 + TEXCOORD_2` or `TEXCOORD_2 + TEXCOORD_3` render through the two existing backend lanes when the needed accessors are readable; material variant patches carry both `uvs` and `uv1` lane changes. 2026-06-22 direct-core PT follow-up: pt-webgpu and pt-webgl2 material packers still warn/drop direct `TextureRef.texCoord > 1` maps instead of silently sampling UV0/UV1. | Native arbitrary UV-array support remains future core/backend contract work. Material routing that needs more than two UV sets, or missing high-UV accessors, emits structured `ignored-material-texcoord` / compatibility issues and drops the affected texture fields instead of silently sampling the wrong UV channel. |
...
SPPM, safe-default BDPT, safe-default ReSTIR-PT result JSONs, the opt-in glossy
ReSTIR-PT research finding, ReSTIR-PT specialty fixture, pt radiometric
host-status JSON, and walkaround host-status JSON machine-checkable without a
new GPU recapture.
...
walkaround host-status/result marker. **2026-06-21 reconciliation:** `npm run
radiometric-ab:pt-host-status` now recaptures SPPM, safe-default BDPT, and
ReSTIR-PT as a native-Deno full-tier `PASS` status artifact; the current
committed walkaround native-Deno radiometric host status records a completed
full-suite `PASS-PARTIAL` rather than `HOST-BLOCKED`. SUN is `PASS`, while
GLASS is `PASS` and GLOSSY is a material-effect `FINDING` rather than
promotion evidence. **2026-06-22 follow-up:** the pt ReSTIR-PT glossy research
branch is now likewise committed as a `FINDING` artifact, not an unmeasured
tail. Same-day walkaround follow-up: `walkaround-ab-all-spp64.json` and
`walkaround-ab-all-spp64-status.json` preserve a 128×128×64-SPP all-cases
native-Deno recapture; it remains `PASS-PARTIAL` because GLOSSY is still a
non-promotable `FINDING`. This is still snapshot/status evidence; browser or
...
| ~~PTWG-VOLUME-DIRNEE-01~~ ✅ code/proof closed | `volumetricSss.test.ts` | In-medium directional NEE over packed N-directional records now has an arithmetic oracle: two RGB directional lights plus a zero-mean gated record are accumulated as `throughputInMedium * irradiance.rgb * hgPhase(dot(ray.direction, towardLight), g)` with literal radiance expectations. 2026-06-22 source guard: the in-medium branch now also decodes signed `angularDiameter` and routes soft directional lights through `sampleDirectionalCone(&rng, dDirAD.xyz, angDiam)` instead of hard-normalizing the light axis. This proves the no-occluder packed-light estimator and pins soft-directional parity; shadowed volume visibility remains render/A-B validation. |
...
2026-06-17 pt-webgl2 browser status, refreshed 2026-06-20:
`tools/gltf-browser-proof/` now drives the
...
assets. The 2026-06-20 source fix makes selected-scene feature reports inspect
reachable `bufferViews[*].extensions.KHR_meshopt_compression` and keep
reachable `KHR_mesh_quantization` accessors in the scoped extension report, so
...
gap look like a renderer failure. The behavioral lane can still fail closed this
way. The radiometric walkaround lane is no longer recorded as host-blocked in the
current committed status: `tools/radiometric-ab/walkaround-ab-host-status.json`
now records `PASS-PARTIAL` for a completed full-suite run and points at
`tools/radiometric-ab/walkaround-ab-results.json`. The preserved result snapshot
remains do-not-promote: SUN is `PASS`, GLASS is `PASS`, and GLOSSY is a
material-effect `FINDING` that proves the path is live without promoting
rich-material GI. `tools/radiometric-ab/walkaround-ab-all-spp64-status.json`
adds a separate 128×128×64-SPP all-cases recapture with the same promotion
boundary: A8 `NEGLIGIBLE`, SUN `PASS`, GLASS `PASS`, GLOSSY `FINDING`.
...
sharded into eleven committed per-label artifacts:
`behavioral-gate-dzn-wh-{default,rcenabled,ppgenabled,gtao-off,checkerboard,skinned-mesh,hdri-env,rect-area-emitter,directional-sun,glass-gi,transparent-oit}-status.json`.
...
| Alpha | alphaMode, alphaCutoff, opacity, alphaMap | Scalar + alpha-map cutout code-closed in 3C/3D; fractional blend camera composition code-closed via transparent OIT with direct sun plus analytic point/spot and four-sample fixed-stratified finite-emitter lighting, plus alpha-aware direct shadow transmittance; point/spot and finite-emitter transparent shadows now carry per-channel Beer/material glass tint. DDGI probe direct sun/point/area-emitter visibility now samples readable baseColor/alpha-map coverage plus `COLOR_0.a` vertex alpha for blend/mask transmittance; shade/ReSTIR-DI/ReSTIR-GI/NRC/GRIS shadow visibility now evaluates readable atlas alpha coverage as binary/transmittance blocker filtering. ReSTIR-DI/GI primary and reconnection vertices now use the opaque-only first-hit predicate so fractional blend surfaces do not enter reservoirs under a different stochastic contract than shade. The structured `alpha-blend-approximation` warning is now coverage-aware: RGB-only readable `baseColorMap` payloads with scalar opacity/base alpha 1 no longer warn as transparent OIT, while RGBA/fractional alpha, `alphaMap`, scalar opacity/baseColor alpha, and vertex `COLOR_0.a` still warn. Transparent ReSTIR direct-light reservoir participation and GI transport remain intentionally approximate/unsupported until a true layered-transport model lands. |
...
**pt-webgpu ledger residuals:** `extensions` is ledger-unsupported but host-discretionary and intentionally warning-free. `displacementMap`, `displacementScale`, and `displacementBias` are now approximate: full-tier `packSceneFromCore` and lite/merged paths apply CPU-readable height maps before BLAS/BVH construction, while scalar displacement material patches trigger a geometry-safe repack. Approximate fields are `shadingModel`, `thickness`, `thicknessMap`, `clearcoatMap`, `clearcoatRoughnessMap`, `clearcoatNormalMap`, `sheenColorMap`, `sheenRoughnessMap`, `iridescenceMap`, `iridescenceThicknessMap`, `specularColorMap`, `specularIntensityMap`, `specularIntensity`, `specularColor`, and the displacement trio; `frontLayer` and `backLayer` are native in the full tier after face-selected layer-normal descriptor/shader support, while the lite tier still emits structured compatibility warnings. `emitterCastShadow` is native in the shadow matrix after SPPM photon-source selection reached no-shadow parity.
...
| `attachVitrum` auto-recreate scene loss | `packages/engine/src/lifecycle/vanilla.ts`; `packages/engine/src/__tests__/attachVitrumAutoRecreate.test.ts` | ✅ DONE (Wave 1 + 2026-06-18 fast-path follow-up + 2026-06-19/20 warning pins): lifecycle tracks scenes submitted through the exposed engine handle and now snapshots `engine.getScene()` immediately before device/context-loss teardown, so add/remove/update/controller fast paths that mutate the backend-retained scene are preserved across recreate too. Regression tests simulate fatal `device-lost` for explicit `setScene`, backend-retained live-scene, snapshot-throw cases, and custom/supplied engines with no live scene snapshot API. The throw case verifies `onError({ phase:'attach:auto-recreate', recoverable:true })` fires and the second `createEngine` call still receives the last tracked scene; the no-`getScene` case emits structured `attachVitrum.auto-recreate-scene-snapshot-unavailable` through `onWarning` before recreating from the latest tracked scene. |
...
2. Compressed wide BVH traversal (`WBVH-01`): the shared substrate is now
   ...
   2026-06-23 broader dzn recapture proves exact parity on material-lobes,
   material-lobe maps, and the glTF material sweep. Timing is uniformly slower
   for CWBVH on the committed dzn promotion shards: simple, complex,
   material-lobes, material-lobe maps, and glTF material sweep. Remaining work
   is browser/real-adapter default-promotion throughput evidence.
...
Walkaround **100%** and arbitrary glTF **100%** are not the same: arbitrary glTF routes rich assets to PT backends via the planner; walkaround 100% still means permanent `unsupported` for spectral/volume/thin-film/full-layer-stack domains with explicit rejection, while front/back absorption layers and displacement are approximate rather than path-tracer-equivalent.
...
complex). 2026-06-23 broader-workload recapture: `pt/cwbvh-broader-material-lobes`,
...
That timing is uniformly slower for CWBVH on the committed dzn promotion shards
(`ratio=1.250` material-lobes, `ratio=1.271` material-lobe maps,
`ratio=1.154` glTF material sweep), so
...
The bounded Road repeat campaign is now committed as a completed five-sample
warmup-discarded dzn artifact across all five workloads:
...
`classification:"uniform-slower"` and `promotion.defaultReady:false`: binary,
complex, material-lobe, and glTF workload medians are slower, and the single
material-lobe-map fast outlier does not make the full workload set default-ready.
The current artifacts prove opt-in same-scene correctness on lavapipe and dzn,
with dzn showing slower median CWBVH timing on the committed promotion shards,
so the default remains binary traversal. Becomes decisive if/when a WebGPU
ray-tracing extension ships (whole-field handicap today: no RT cores in the
browser for anyone).
...
`*.denoiser-auto-resolved` warnings with
`packageProvidesProductionWeights:false`, and all three ledger `auto` as native
resolver support. **Remaining work:** license-vetted OIDN weight distribution
(or train the in-repo UNet to production quality), wasm/webgpu execution path
that needs no host wiring, and quality A/B before any default-on production
neural claim.
...
was pulled forward into the `VQ-WALKAROUND-RADIOMETRIC-AB` partial-proof validation
row** (converged-unbiasedness of GRIS-on + measured error of the biased default).
