...
| ReSTIR-PT | `restirPtProducer.wgsl.ts`, `restirPtCompose.wgsl.ts`, `reservoirPtHero.wgsl.ts`, `restirPtResolve.wgsl.ts` | ✅ producer direct/onward paths use full helpers; ✅ scalar-lobe reservoir payload/target/resolve now uses full visible-domain helpers, including `KHR_materials_specular` scalar colour/intensity and `clearcoatNormalV` (`ReservoirPTHero` 56-word layout); ✅ visible-vertex payload now mirrors the main shade prologue for alpha pass-through, baseColor/AO/roughness+metallic/normal/bump/clearcoat-normal/transmission/extension scalar maps, layer tint/roughness, thin-film, and spectral albedo; ✅ suffix/reconnection vertices now alpha-skip and decode the same hit-local material-map/layer/thin-film/spectral domain before Lo evaluation, using the mapped suffix normal for the reservoir geometry and mapped clearcoat normal for clearcoat BRDF/PDF paths; ✅ producer source sampler now samples a normalized base/clearcoat/sheen lobe mixture and stores the matching `pdfSrc`, with the clearcoat lobe sampled/PDFed against the mapped clearcoat normal. 2026-06-16 proof wave: `tools/radiometric-ab/ab-restir-pt-specialty.mjs` + `restirPtSpecialtyReference.test.ts` pin the one-sample producer/finalize/resolve identity for clearcoat, sheen, iridescence, and anisotropy payloads. |
| Adjoint | `adjointPass.wgsl.ts`, `pathTraceAdjoint.wgsl.ts` | ◻/✅ `req.samples` is consumed by replaying the frozen inverse sample sequence and averaging gradients. 2026-06-15 follow-up: path-replay now differentiates KHR_materials_specular `specularColor` / `specularIntensity`, reads packed material vec4 #27, and downgrades to finite difference for material/light domains the adjoint pass cannot honestly mirror. 2026-06-16 follow-up: runtime selection now covers delta directional, point, spot, and stochastic area-measure rect/disc/mesh-area direct lights; the adjoint pass binds packed directional/spot/mesh-area buffers, honors point/spot/rect/disc/mesh emitter shadow-disable lanes, and uses native disc area plus packed mesh-triangle area terms. Later 2026-06-16 follow-up: scalar `metallic` now has a CPU-FD-checked opaque BRDF partial, WGSL path, engine field code, and inverse-session routing in the same scoped direct-light domain. Same-day follow-ups: scalar `emissiveIntensity` now shares RGB `emissive`'s primary-hit emission adjoint; hit-local `emissiveMap` texels now modulate both emissive gradients via the forward per-map UV/transform/wrap sampler subset; baseColorMap/COLOR_0/AO, roughness/metallic maps, specular maps, clearcoat/sheen maps, iridescence/thickness maps, and anisotropy maps now replay their local direct-light chain/state factors; map-free scalar `clearcoat` / `clearcoatRoughness` now share the direct-light adjoint domain through CPU-FD-checked additive clearcoat-lobe partials; map-free `sheen` / `sheenColor` / `sheenRoughness` now share that domain through CPU-FD-checked additive Charlie-lobe partials; and map-free scalar `iridescence` now differentiates the thin-film-modified base F0 in the same direct-light slice. Inverse-session routing also rejects primitive targets the replay shader cannot truthfully intersect as raw triangles (analytic primitives, instanced meshes, and transformed meshes) instead of silently selecting path replay. Scalar `transmission`, `thickness`, `opacity`, and `alphaCutoff` are now admitted to finite-difference inverse sessions with structured path-replay downgrade diagnostics. Newer 2026-06-16 follow-ups: deterministic direct emitter `color` / `intensity` path-replay is wired for delta directional, point, spot, rect-area, disc-area, and uncapped explicit mesh-area targets; the descriptor carries kind-local light slot/range plus unfactored color/intensity, and the shader scatters through the scoped receiver BRDF value and per-light attenuation/geometric terms. Latest 2026-06-16 follow-up: finite area replay no longer uses center points; rect/disc emitters use Shirley-Chiu/rect surface samples and mesh-area emitters use uniform triangle barycentrics with solid-angle area PDFs. Soft directional, capped/reordered or unmapped mesh-area emitter targets, unreplayed receiver materials, and unsupported lighting still downgrade explicitly. Still OPEN for full-path parity because alpha-map/alpha-visibility adjoints, normal/bump/light/transmission/thickness/displacement and clearcoat-normal maps, layered/volume/spectral material domains, environment/soft-sun light terms, exact texel-PDF mesh emission, forward light-selection MIS parity, indirect paths, GPU inverse-fit recaptures, and full material-lobe A/B proofs remain outside this scoped adjoint. |
| Present | `present.wgsl.ts` tonemap only — no BSDF | N/A |

**Footgun:** Fixing megakernel only used to leave BDPT/SPPM/ReSTIR-PT wrong for glTF clearcoat scenes on specialty paths. The clearcoat-normal map now reaches the megakernel, BDPT light-side surface vertices, and ReSTIR-PT reservoir/resolve/source-PDF/suffix-Lo paths; BDPT's light-side material domain now also applies layer tint/roughness, thin-film reflect tint, Cauchy IOR, and spectral reflectance scalar. ReSTIR-PT now has a deterministic CPU/static specialty-lobe identity fixture, and map-free scalar clearcoat, sheen, and iridescence controls have scoped direct-light adjoint partials, but the remaining class still includes clearcoat maps/normal maps, sheen maps, iridescence maps/IOR/thickness gradients, anisotropy inverse-adjoint derivatives, plus GPU material-furnace/reference-render promotion, not just missed function calls.
