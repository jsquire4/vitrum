...
| `materialPatch` fast path | ✅ FIRST SLICE: `HybridEnginePrimitiveUpdates.ts` | Scalar-only material edits keep the slice upload path; `baseColorMap` handle/UV/wrap/transform changes route through full rebuild so atlas metadata cannot go stale. A narrower atlas refresh remains an optimization follow-up. |
| Ledger | ✅ FIRST SLICE: `WALKAROUND_MATERIALS`, `CONSUMED_MATERIAL_FIELDS` | `baseColorMap` promoted to `approximate` with tests. Remaining maps remain unsupported until each has shader consumption. |
