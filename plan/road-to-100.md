...
`roughnessMap`/`metallicMap`/`aoMap`/`alphaMap`/`emissiveMap` (2026-06-14) now have real walkaround paths for
...
hits, and emissive maps modulate camera-visible emitter glow. `CONSUMED_MATERIAL_FIELDS` and the
core promise ledger now grade walkaround `baseColorMap`, `roughnessMap`,
`metallicMap`, `aoMap`, `aoMapIntensity`, `alphaMap`, and `emissiveMap` as
...
| Material index per tri | ✅ SECOND SLICE: metadata texture keyed by triangle index | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, `alphaMap`, and `emissiveMap` use `triangleMaterialIds` at pack time; scalar lanes stay as fallback when no readable map exists. |
...
| Ledger | ✅ SECOND SLICE: `WALKAROUND_MATERIALS`, `CONSUMED_MATERIAL_FIELDS` | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, `aoMapIntensity`, `alphaMap`, and `emissiveMap` promoted to `approximate` with tests. Remaining maps remain unsupported until each has shader consumption. |
