...
| **Texture wrap modes (ADDED 2026-06-12 — verified hole)** | ~~`TextureRef.wrapS/wrapT` in core (`repeat`/`clamp`/`mirror`) + adapter reads `gltf.samplers`~~ ✅ API DONE (`material.ts`, `textures.ts`, `gltfTextureSweep.test.ts`, `gltfAssetApi.test.ts`, `texturePipeline.ts`). ✅ pt-webgpu full-tier material textures now pack per-map wrap pairs (`materialTextures.ts`) and WGSL applies repeat/clamp/mirrored-repeat before UV-fit scaling (`material.wgsl.ts`). Still open: pt-webgl2 / walkaround sampler/wrap audit and shader/per-layer strategy where one array sampler cannot vary by texture. | API no longer drops sampler semantics and `textureDecodeReport` exposes wrap modes. Runtime parity for clamp/mirror/repeat remains required before visual Khronos-sweep signoff on the remaining backends; KHR_texture_transform scale>1 can still render incorrectly there if a backend forces one wrap policy. |
...
| ~~`clearcoatMap`, `clearcoatRoughnessMap`, `sheen*Map`, `iridescence*Map`, `specular*Map`~~ ✅ DONE/approx | `materialTextures.ts` extends the full-tier descriptor stride to 63 vec4s, packs these extension-lobe maps into the correct sRGB/linear arrays, records per-map texCoord / KHR_texture_transform / wrap / UV-fit data, and `material.wgsl.ts` samples the glTF channels before `shadePrologue.wgsl.ts` modulates decoded lobe parameters. Tests: `materialTextures.test.ts`, `wgslContract.test.ts`, `liteTierCapabilities.test.ts`; `shader-gate --self-test` and WSL `validate:gpu:smoke` pass. | Still `approximate` in the promise ledger because ReSTIR-PT visible-vertex texture-map parity, clearcoat/sheen source-lobe sampling/PDF schemas, and BDPT light-subpath scatter PDFs still need dedicated sampler/payload work. `clearcoatNormalMap` / `clearcoatNormalScale` remain unsupported. |
| ~~`specularIntensity`, `specularColor`~~ ✅ DONE/approx | Packed in material vec4 #27 and consumed by ordinary PT BRDF/PDF paths, MNEE/SPPM receiver paths, BDPT light-subpath surface scattering, and ReSTIR-PT visible-domain reservoir/resolve payloads. Tests: `scenePack.materials.test.ts`, `wgslContract.test.ts`, `wgslLiteContract.test.ts`, `bdptGlossyLightSubpath.test.ts`; `shader-gate --self-test` compiles production shaders. | Still `approximate` in `BACKEND_PROMISE_LEDGER` until the remaining source-lobe sampler/PDF and legacy specialty schemas have the same scalar/material-lobe coherence |
...
| Emitter `castShadow` | DDGI/ReSTIR/RC direct-light paths | ✅ CODE PARTIAL / ledger approximate: analytic point/spot payload packs binding-13 lane `[13]`; shared `EmitterTri` packs lane `[19]`; ReSTIR-DI candidate visibility + shade visibility gate on `e.castShadowDisabled`; DDGI/RC area-emitter NEE skip the emitter shadow ray; `HybridEngine` warns with explicit residual paths. | Required for native: add the flag to DDGI/RC point/spot fixture-light structs plus directional sun paths; keep `WALKAROUND_SHADOWS.emitterCastShadow = 'approximate'` until those paths are covered. |
