...
walkaround host-status/result marker. **2026-06-21 reconciliation:** the current
committed native-Deno radiometric host status now records `PASS-PARTIAL` rather
than `HOST-BLOCKED`: the targeted SUN run completed with exit 0 and SUN is
`PASS`, while GLASS remains smoke-only. GLOSSY is a material-effect `FINDING`
rather than promotion evidence. This is still snapshot/status evidence;
...
now records `PASS-PARTIAL` for the targeted SUN run and points at
