...
2026-06-20 sampler-policy truthfulness follow-up: core/glTF `TextureRef.magFilter` / `minFilter` / `mipFilter` now reach runtime diagnostics instead of disappearing after compatibility analysis. pt-webgl2 emits `pt-webgl2.texture-sampler-policy-approximation` when a material-map ref requests anything outside its shared nearest/no-mip atlas sampler. pt-webgpu carries sampler-policy metadata through `MaterialTextureLayerUse` and emits `pt-webgpu.material-texture-sampler-policy-approximation` when a layer requests anything outside its shared linear/linear/mipmap-linear material-texture sampler. This does **not** promote exact per-texture filtering/mipmap parity; it makes the bounded approximation visible to core-scene and glTF hosts until a real per-policy shader/sampler implementation lands.
