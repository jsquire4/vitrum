...
> plus roughnessMap/metallicMap local chain factors, plus transmission, layered,
> spectral/volume, soft-sun, environment,
...
> baseColorMap/COLOR_0 and roughnessMap/metallicMap local chain factors for
> scoped pt-webgpu path-replay fits, made transparent-OIT direct sun
...
texel-exact emitter PDFs, analytic/extra emitter mapped payloads plus GI/RC/DDGI
...
| ~~Multiple UV sets~~ ✅ POLICY/TEST CLOSED | `TEXCOORD_1` imports/consumes as `uv1`; generated tangent frames now use `TEXCOORD_1` when tangent-space maps select `texCoord:1`; `TEXCOORD_2+` material textures and generated-tangent requests produce structured `unsupported`/`missing-tangent-texcoord` compatibility issues with source paths, pinned by `gltfKhronosSweep.test.ts` and `gltfAdapter.test.ts`. | Native UV-set-2+ sampling remains future contract work because core `Scene` carries only `uvs` / `uv1`; arbitrary-glTF strict modes reject before render instead of silently sampling the wrong UV set. |
...
| Alpha | alphaMode, alphaCutoff, opacity, alphaMap | Scalar + alpha-map cutout code-closed in 3C/3D; fractional blend camera composition code-closed via transparent OIT with direct sun plus analytic point/spot and center-sampled finite-emitter lighting, plus alpha-aware direct shadow transmittance; transparent ReSTIR direct-light reservoir participation and GI remain approximate |
