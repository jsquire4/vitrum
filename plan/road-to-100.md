...
**Footgun:** Fixing megakernel only used to leave BDPT/SPPM/ReSTIR-PT wrong for glTF clearcoat scenes on specialty paths. The clearcoat-normal map now reaches the megakernel, BDPT light-side surface vertices, and ReSTIR-PT reservoir/resolve/source-PDF/suffix-Lo paths. The remaining class is inverse/adjoint derivatives, BDPT light-side layer/thin-film/spectral special cases beyond the row-4 material payload, and proof/A-B coverage, not just missed function calls.
