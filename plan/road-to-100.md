...
> D12 (shared-bvh `worldSpaceMerge` uv1 zero-triangle desync).**
...
- **D11 — pt-webgl2 declared-but-never-uploaded uniforms (inert features) (ADDED 2026-06-13 — lead-verified audit)** ◻: (a) the global homogeneous volume medium `u_volumeDensity`/`u_scatterAlbedo`/`u_anisotropyG` is declared at `composeTraceGlsl.ts:401-403` and consumed by the scatter branch at `:668-690`, but has **no TS setter** anywhere in `pt-webgl2/src` (repo-wide search hits only GLSL + comments) → defaults to 0 → the volume-march branch is unreachable; (b) `materialLodDepth` is declared at `glsl/render/get_surface_record_function.glsl.js:11` and gates `useTextures` at `:53`, with no TS setter → always 0 → the per-depth texture-LOD optimization never activates. (Per-material SSS via `surf.sssSigmaT` IS wired and works — only the *global* medium is dead.) **Fix:** classify each truly-dead vs unwired-intended with evidence, then either wire it end-to-end (host uniform upload + options/frame plumbing) or remove the dead GLSL and document the omission in `featureTypes.ts` alongside the other pinned-off gates. Do not leave a declared-and-consumed-but-never-set uniform. **Effort: S–M.**
