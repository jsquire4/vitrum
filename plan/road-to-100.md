...
| ~~`anisotropy` / `anisotropyRotation` / `anisotropyMap`~~ ✅ DONE (2026-06-13; dispersion follow-up 2026-06-20) | Scalar strength/rotation pack into reserved lanes `s11.a` / `s17.b`; `anisotropyMap` packs into `s6.b`, transform texels `95..96`, UV bit 19, and the final wrap-mode pair. GLSL decodes the map and samples RG/B according to KHR_materials_anisotropy (`B` strength, `RG` rotation offset), feeding anisotropic GGX sampling/eval/PDF in `bsdf_functions.glsl.js`. 2026-06-20 follow-up: pt-webgl2 Cauchy/hero-wavelength dispersion transmission now uses the same `ggxDirectionForSurface()` visible-normal helper as ordinary transmission, so anisotropic glass does not become isotropic only when dispersion is active. Tests: `materialsTexture.test.ts`, `materialStrideParity.test.ts`, `engineContract.test.ts`, `bsdfLobeSelectionPolicy.test.ts`, core ledger contract. |
