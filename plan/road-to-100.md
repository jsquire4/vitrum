...
2026-06-20 sampler-policy truthfulness follow-up: core/glTF `TextureRef.magFilter` / `minFilter` / `mipFilter` now reach runtime diagnostics instead of disappearing after compatibility analysis. pt-webgl2 emits `pt-webgl2.texture-sampler-policy-approximation` when a material-map ref requests anything outside its shared nearest/no-mip atlas sampler. Exact pt-webgl2 per-map sampler policy remains a real code tail because the current `sampler2DArray` atlas has one common nearest-filtered, no-mip payload; closing that would require a mipmapped/manual-sampling atlas or a different material-texture representation, not a warning-only tweak. walkaround-hybrid emits `walkaround-hybrid.material-texture-sampler-policy-approximation` when an atlas-backed material map authors mag/min/mip policy that the shared material atlas sampler approximates; the map remains atlas-backed, and the warning carries source path plus sampler metadata. pt-webgpu now appends per-map mip-policy and mag/min filter-policy lanes to the full-tier material texture descriptor; the WGSL explicit-LOD sampler honors authored `mipFilter:"none"`, nearest mip selection, linear mip interpolation, and nearest-vs-linear mag/min policy for regular material maps through a `textureLoad` nearest branch plus the existing filtered `textureSampleLevel` branch. The pt-webgpu adjoint replay pass now consumes those same descriptor lanes for replayed material maps. Runtime/static diagnostics now warn only for the bump-map raw finite-difference path's non-linear or explicitly mipmapped policies, because that path samples base texels in raw UV space for height gradients. Remaining pt-webgpu/walkaround sampler-parity proof belongs to renderer A/B validation, not hidden API/data loss.
