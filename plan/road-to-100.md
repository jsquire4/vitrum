...
`oidnModelUrl`, then the preset/default denoiser, and emits
`walkaround-hybrid.denoiser-auto-resolved` with
`packageProvidesProductionWeights:false`; PT backends ledger it as unsupported
and warn/degrade like other non-wired denoisers. **Remaining work:**
license-vetted OIDN weight distribution (or train the in-repo UNet to production
quality), wasm/webgpu execution path that needs no host wiring, and quality A/B
before any default-on production neural claim.
