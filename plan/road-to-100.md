...
> beyond its current base/ORM/AO/alpha slices; H-residue (H5/H21/H24-cluster/H32/H34/H35).
...
`roughnessMap`/`metallicMap`/`aoMap` (2026-06-14) now have real walkaround paths for
...
before multiplying the runtime GTAO factor. `CONSUMED_MATERIAL_FIELDS` and the
core promise ledger now grade walkaround `baseColorMap`, `roughnessMap`,
`metallicMap`, `aoMap`, and `aoMapIntensity` as `approximate`. They are deliberately not
`native`: glass Beer/transmission tint plus upstream reservoir/GI payloads still
use scalar packed lanes, and the rest of the texture-map family is still open.

| Component | File(s) | Notes |
|-----------|---------|-------|
| Atlas build | ✅ SECOND SLICE: `pipeline/materialTextureAtlas.ts` | `baseColorMap` raw/DataTexture handles are inverse-sRGB decoded into linear RGBA32F array layers; `roughnessMap`, `metallicMap`, `aoMap`, and `alphaMap` are packed as linear scalar-map layers and sampled from glTF G/B/R/R channels. Remaining maps still need atlas rows/channel policy. |
...
| Material index per tri | ✅ SECOND SLICE: metadata texture keyed by triangle index | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, and `alphaMap` use `triangleMaterialIds` at pack time; scalar lanes stay as fallback when no readable map exists. |
...
| Ledger | ✅ SECOND SLICE: `WALKAROUND_MATERIALS`, `CONSUMED_MATERIAL_FIELDS` | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, `aoMapIntensity`, and `alphaMap` promoted to `approximate` with tests. Remaining maps remain unsupported until each has shader consumption. |
...
**pt-webgl2:** 9 unsupported → displacement-only residuals; `thicknessMap` is implemented as an approximate volume attenuation clamp.
