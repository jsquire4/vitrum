...
> are exact texel-PDF mesh emission promotion, sampled one-of-N light-selection
> replay/promotion under sampled multi-light selection, environment/indirect transport, and unreplayed
> receiver material domains, which still downgrade with structured diagnostics.
> Same-day proof follow-up: the sampled-selection downgrade is now pinned through
> the real `createPTEngine_WebGPU()` inverse-session path (`inversePausedRender`
> sets a point light + HDRI environment and asserts
> `path-replay-unsupported-light-selection`), not only through fake session hooks.
