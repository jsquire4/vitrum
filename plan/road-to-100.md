...
Adjoint alpha note (2026-06-20): the remaining alpha-map tail above means active mask/blend visibility. Opaque-material `alphaMap` is now code-closed for path replay because forward `alphaTestPassThrough` returns before sampling alpha when `alphaMode:"opaque"`; `inverseSession.test.ts` pins both dormant-alpha replay and active-alpha finite-difference downgrade.
