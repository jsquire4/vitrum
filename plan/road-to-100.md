...
Adjoint dormant-map note (2026-06-20): the remaining alpha-map tail above means active mask/blend visibility. Opaque-material `alphaMap` is now code-closed for path replay because forward `alphaTestPassThrough` returns before sampling alpha when `alphaMode:"opaque"`. Transmission/thickness maps with scalar `transmission` at zero are likewise code-closed for opaque direct-light path replay; active transmissive transport still downgrades. `inverseSession.test.ts` pins dormant-alpha/dormant-transport replay plus active-alpha/active-transport finite-difference downgrade.
