...
committed native-Deno radiometric host status is `HOST-BLOCKED` on the known
wgpu-hal panic, and the proof checker preserves the do-not-promote warning
alongside the previous walkaround result snapshot. The preserved SUN lane remains
partial, GLASS remains smoke-only, and GLOSSY is a material-effect `FINDING`
...
gap look like a renderer failure. The radiometric walkaround lane currently
records the same native-Deno host class: `tools/radiometric-ab/walkaround-ab-host-status.json`
is `HOST-BLOCKED` on the known wgpu-hal panic and preserves
`tools/radiometric-ab/walkaround-ab-results.json` for continuity. The preserved
result snapshot remains do-not-promote: SUN is partial, GLASS is smoke-only, and
GLOSSY is a material-effect `FINDING` that proves the path is live without
promoting rich-material GI.
