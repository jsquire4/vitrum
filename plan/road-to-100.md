...
`roughnessMap`/`metallicMap`/`aoMap`/`alphaMap`/`emissiveMap`/`transmissionMap` (2026-06-14) now have real walkaround paths for
...
before multiplying the runtime GTAO factor. Alpha maps cut out primary/RIS/GI
...
`metallicMap`, `aoMap`, `aoMapIntensity`, `alphaMap`, `emissiveMap`, and `transmissionMap` as
...
|-----------|---------|-------|
| Atlas build | ✅ SECOND SLICE: `pipeline/materialTextureAtlas.ts` | `baseColorMap` and `emissiveMap` raw/DataTexture handles are inverse-sRGB decoded into linear RGBA32F array layers; `roughnessMap`, `metallicMap`, `aoMap`, `alphaMap`, and `transmissionMap` are packed as linear scalar-map layers and sampled from glTF G/B/R/R/R channels. Remaining maps still need atlas rows/channel policy. |
...
| Material index per tri | ✅ SECOND SLICE: metadata texture keyed by triangle index | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, `alphaMap`, `emissiveMap`, and `transmissionMap` use `triangleMaterialIds` at pack time; scalar lanes stay as fallback when no readable map exists. |
...
| Ledger | ✅ SECOND SLICE: `WALKAROUND_MATERIALS`, `CONSUMED_MATERIAL_FIELDS` | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, `aoMapIntensity`, `alphaMap`, `emissiveMap`, and `transmissionMap` promoted to `approximate` with tests. Remaining maps remain unsupported until each has shader consumption. |
