...
Scalar `specularColor` / `specularIntensity`, readable `specularColorMap` / `specularIntensityMap`, scalar `clearcoat` / `clearcoatRoughness`, and scalar `sheen` / `sheenColor` / `sheenRoughness` are now code-closed as approximate: material-atlas metadata stores the scalar controls/maps and shade-owned direct, analytic, sun, and glossy-indirect paths consume them. Remaining 3E work is iridescence, anisotropy, clearcoat maps/normal maps, sheen maps, and any ReSTIR/GI payload/PDF parity needed for promotion beyond approximate.
...
| Disney scalars | iridescence*, anisotropy*, clearcoat maps, sheen maps | 3E; scalar/specular-map `specularColor`/`specularIntensity`, scalar `clearcoat`/`clearcoatRoughness`, and scalar `sheen`/`sheenColor`/`sheenRoughness` are approximate in shade-owned GGX paths |
