...
| ~~Morph `TANGENT` contract policy~~ ✅ DONE/approx | `SkinnedMeshPrimitive.morphTargetTangents`, `skinSolver.ts`, `gltfToScene.ts`, `featureReport.ts`, `gltfModesMorphsAnimations.test.ts`, `gltfAssetApi.test.ts`, `skinnedMeshIngestion.test.ts`, `pt-webgl2/src/scene/solveSkinPrimitives.ts`, `skinSolve.test.ts` | glTF morph-target TANGENT VEC3 deltas are preserved on the core primitive contract and the shared CPU skin solver now applies them to solved tangent streams when rest tangents exist. pt-webgl2 now preserves that posed tangent stream during skinned ingestion instead of dropping it before attribute packing. Compatibility remains `approximate` because GPU-native tangent skinning and broader backend evidence are still validation/promotion work rather than full native compute-kernel coverage. |
