...
| RC exported-surface footguns | `cascadePyramid.ts`; `cascadeDispatch.ts`; `HybridEngineRC.ts`; `cascadeDimsOverride.test.ts`; `rcMergedRefit.test.ts` | ✅ DONE (Wave 4 residue): `validateCascadeDims()` rejects empty/malformed overrides, non-positive probes, non-square ray counts, broken 2× ray-grid steps, and invalid intervals before allocation/dispatch. `RCSubsystem.refitCascadeBounds()` now invalidates dispatcher bindings when probe bounds change, so merge uniforms rebuild with fresh `probeOriginWorld`/`roomSize`; merged-instance refit test pins invalidation without dispatcher recreation. Direct raw-dispatch `bvhMode` transitions remain an H37 direct-caller lifecycle item, not this Road row. |
