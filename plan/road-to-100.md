...
`normalMap`/`roughnessMap`/`metallicMap`/`aoMap`/`alphaMap`/`emissiveMap`/`transmissionMap`/`lightMap` plus specular, clearcoat factor/roughness/normal, sheen color/roughness, anisotropy, and iridescence factor/thickness maps (2026-06-14) now have real walkaround paths for
...
paths still use scalar packed lanes, and bump, thickness, and displacement map families are still open or deliberately unsupported.
...
| Disney scalars | specular*, clearcoat*, sheen*, anisotropy*, iridescence* | 3E; these rows are approximate in shade-owned GGX paths; native promotion still needs ReSTIR/GI payload/PDF parity and authored tangent parity where applicable |
