...
> (`bumpScale`, `clearcoatNormalScale`) while requested path replay downgrades
> them explicitly until analytic normal terms are mirrored; top-level
> `normalMap`/`normalScale` is now mirrored in the scoped direct-light adjoint
> path, including the forward shader's authored/generated tangent.xyzw
> handedness before UV-gradient fallback.
