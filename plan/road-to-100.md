...
| **B9** | ✅ **DONE (Wave B) — isotropic/base GGX multiscatter (all 3 backends)** | pt-webgpu `material.wgsl.ts`; pt-webgl2 `glsl/render/get_surface_record_function.glsl.js`; walkaround `ggxBrdf.wgsl.ts` | **Implemented:** Kulla-Conty multiscatter energy compensation in all three base GGX evals (LUT + furnace test on pt-webgpu; furnace-pinned on pt-webgl2, lite-mode skipped). The pt-webgpu anisotropic sampler/eval/PDF are live, but anisotropic multiscatter remains an explicit approximate/promotion tail until an anisotropic furnace/reference model lands. | Anisotropic material-furnace/reference promotion remains proof/research, not a silent contract promise. | done (base GGX) / anisotropic promotion tail |
