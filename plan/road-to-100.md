...
|-----------|---------|-------|
| Atlas build | ✅ SECOND SLICE: `pipeline/materialTextureAtlas.ts` | `baseColorMap` raw/DataTexture handles are inverse-sRGB decoded into linear RGBA32F array layers; `roughnessMap`, `metallicMap`, and `aoMap` are packed as linear scalar-map layers and sampled from glTF G/B/R channels. Remaining maps still need atlas rows/channel policy. |
...
| Material index per tri | ✅ SECOND SLICE: metadata texture keyed by triangle index | `baseColorMap`, `roughnessMap`, `metallicMap`, and `aoMap` use `triangleMaterialIds` at pack time; scalar lanes stay as fallback when no readable map exists. |
...
| Ledger | ✅ SECOND SLICE: `WALKAROUND_MATERIALS`, `CONSUMED_MATERIAL_FIELDS` | `baseColorMap`, `roughnessMap`, `metallicMap`, `aoMap`, and `aoMapIntensity` promoted to `approximate` with tests. Remaining maps remain unsupported until each has shader consumption. |
