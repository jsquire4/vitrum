...
> `normalScale` still downgrades when a bump map or clearcoat-normal map is
> present, and `bumpScale` still downgrades when a clearcoat-normal map is
> present, because those chained derivatives through nested tangent frames are
> not implemented.
