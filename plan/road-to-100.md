...
Adjoint dormant-map note (2026-06-20): the remaining alpha-map tail above means active mask/blend visibility. Opaque-material `alphaMap` is now code-closed for path replay because forward `alphaTestPassThrough` returns before sampling alpha when `alphaMode:"opaque"`. Same-day scalar-alpha follow-up: map-free `alphaMode:"mask"` stays on path replay when scalar `opacity >= alphaCutoff`, and map-free `alphaMode:"blend"` stays on path replay when scalar `opacity >= 1`, provided the primitive has no non-opaque `COLOR_0.a`; `baseColorMap.a`, `alphaMap`, non-opaque vertex alpha, mask opacity below cutoff, and fractional blend opacity still emit `path-replay-unsupported-visibility`. Transmission/thickness maps plus spectral/dispersion/scattering metadata with scalar `transmission` at zero are likewise code-closed for opaque direct-light path replay, and identity front/back layers, layer-normal-scale metadata without a layer normal map, plus empty thin-film stacks no longer force a downgrade because they pack to the same BRDF inputs as absent layers. Active transmissive/spectral/volume/layered/thin-film transport still downgrades. `inverseSession.test.ts` pins dormant-alpha/dormant-transport/dormant-layer replay plus active-alpha/active-transport finite-difference downgrade.
...
Alpha-visibility inverse classifier note (2026-06-20): `inverseSession.ts` now mirrors pt-webgpu material-texture upload semantics before declaring `baseColorMap.a` a visibility blocker. Readable RGB/one/two/three-channel `baseColorMap` payloads expand to alpha 1 and no longer downgrade otherwise-compatible active `alphaMode` scenes to finite difference; readable RGBA payloads with fractional alpha, standalone `alphaMap`, vertex `COLOR_0.a`, and scalar opacity/cutoff visibility changes still produce `path-replay-unsupported-visibility` until replay mirrors those discontinuous visibility terms.
