...
2026-06-20 sampler-policy truthfulness follow-up: core/glTF `TextureRef.magFilter` / `minFilter` / `mipFilter` now reach runtime diagnostics instead of disappearing after compatibility analysis. pt-webgl2 emits `pt-webgl2.texture-sampler-policy-approximation` when a material-map ref requests anything outside its shared nearest/no-mip atlas sampler. pt-webgpu now appends per-map mip-policy lanes to the full-tier material texture descriptor and the WGSL explicit-LOD sampler honors authored `mipFilter:"none"`, nearest mip selection, and linear mip interpolation for regular material maps while retaining the shared linear mag/min sampler. Runtime/static diagnostics now warn only for non-linear mag/min filtering and the bump-map raw finite-difference path's explicitly mipmapped policies. This does **not** promote exact per-texture magnification/minification filter parity; it makes the remaining bounded approximation visible to core-scene and glTF hosts.
