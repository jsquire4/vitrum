...
> (`normalScale`, `bumpScale`, `clearcoatNormalScale`, `lightMapIntensity`,
> `envMapIntensity`) while requested path replay downgrades them explicitly until
> analytic normal/light/env-map terms are mirrored. `aoMapIntensity` now stays on
> scoped path replay as the local derivative of glTF AO's baseColor multiplier.
