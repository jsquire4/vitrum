...
| Walkaround radiometric A/B harness | walkaround/tools | **UPDATED 2026-06-22.** The SUN fixture now uses the directional-light radiance baseline `Lo = I * cos(theta) * albedo / pi`, runs as diffuse-only, and disables sky/GTAO/denoising for that analytic case. `VITRUM_WALKAROUND_AB_CASES` can rerun selected cases while preserving the other committed results. The harness also exposes width/height/SPP/profile/output overrides for higher-quality case-specific reruns; the committed 64-SPP glossy recapture status fail-closes as `HOST-BLOCKED` on the known native-Deno `wgpu-hal` GLES panic and keeps the do-not-promote warning. Current WSL native-Deno baseline validation completes and records `PASS-PARTIAL`: A8 is `NEGLIGIBLE`, SUN is `PASS`, GLASS is `PASS`, and GLOSSY remains a material-effect `FINDING`; rerun on browser/real-adapter or higher-quality case-specific references before promotion. |
