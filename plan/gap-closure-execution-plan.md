...
| High-UV remap edge cases | glTF adapter / PT backends | `gltfToScene.ts`; texture sweep tests; PT material packer tests | **CLOSED/NARROWED.** Single high `TEXCOORD_N` remaps to `uv1` when lossless; conflict/missing-accessor diagnostics remain. Direct core pt-webgpu/pt-webgl2 material packers now warn/drop `texCoord > 1` maps instead of silently sampling UV0/UV1. Native arbitrary UV arrays are parked as a future contract. |
...
| Walkaround radiometric A/B harness | walkaround/tools | **UPDATED 2026-06-23.** The SUN fixture uses the directional-light radiance baseline `Lo = I * cos(theta) * albedo / pi`, runs as diffuse-only, and disables sky/GTAO/denoising for that analytic case. `VITRUM_WALKAROUND_AB_CASES` can rerun selected cases while preserving the other committed results. The harness also exposes width/height/SPP/profile/output overrides for higher-quality case-specific reruns. Current WSL native-Deno baseline validation completes on the forced lavapipe ICD and records `PASS-PARTIAL`: A8 is `NEGLIGIBLE`, SUN is `PASS`, GLASS is `PASS` with bounded ratios, and GLOSSY remains a material-effect `FINDING`. The 64-SPP all-cases recapture also records GLASS `PASS` and GLOSSY `FINDING`; rerun on browser/real-adapter or higher-quality case-specific references before promoting the remaining rich-material GI blocker. |
...
| Neural denoiser quality | shared/walkaround | **Checkpoint classification/status guard updated 2026-06-22:** every committed `.vitrum-model` is manifest-pinned by role/size/SHA/param count, `learned-systems-proof-check` fails on unregistered or production-like weights without a passing production A/B manifest, and `tools/learned-systems/learned-systems-status.json` records the current `provisioning-needed` posture: 2 research checkpoints, 0 production checkpoints, neural/GRIS/NRC/PPG non-default. Remaining work is still the actual production checkpoint plus quality A/B; otherwise keep opt-in. |
