...
| SSS / translucent panels | approximate | supported | pt-webgl2: `materialsTexture.test.ts` pins `sssSigmaT`/albedo/aniso lanes; `composeTraceGlsl.test.ts` pins the SSS branch; pt-webgpu: `wgslContract.test.ts` (`isTranslucent` gate) | pt-webgl2: runtime A/B capture pending. pt-webgpu dzn (RTX 4090) mixed-panel toggle A/B (re-run-confirmed): SSS LOCALIZED to flagged panel — LEFT/RIGHT Δ-luma ratio 1190:1; baseline `tools/reference-renders/baseline/ptwgpu-sss-mixed-panels.png` | pt-webgl2 uses per-material SSS fields, not the old unassigned `u_volumeDensity` path; still `approximate` because `scatteringCoefficientRGB` is reinterpreted as volume/SSS albedo and lacks visual promotion. |
