...
- **Maturity (do not call the library "pre-alpha"):** release-candidate track for `engine` / `walkaround-hybrid`. **`pt-webgl2`'s RC label was SUSPENDED (2026-06-09 deep audit) — the headline upload-gap cluster is now FIXED + GPU-validated (2026-06-09 validation campaign):** H1 analytic-light NEE (`lights.count` was never uploaded → now uploaded via the new `GlProgram.setUint`; rect-area Cornell black→lit on ANGLE), H2 `spectral` (CMF tables now uploaded; black→lit, luminance-unbiased vs RGB to 0.02%), H3 directly-visible env (`backgroundAlpha` now uploaded; sky black→exact env colour), plus H4 forward-MIS power-weighting — see `items_to_fix.md` §H H1–H4 (FIXED) and `wsl-gpu/.../revalidation-2026-06-09/RESULTS.md`. **Still open before un-suspending:** H5 `bdpt` never host-driven (unbound usampler), H6/H7/H8, and the mock-GL test suite is blind to this upload-gap class (no naga/real-GL gate → these could silently regress). The prior fork-vs-native A/Bs passed because they exercised only the then-working paths (emissive-fold Cornell / glass / IBL-indirect / textures). `pt-webgpu` is a peer PT backend — all 9 of its fidelity-matrix rendering rows are `supported` (dzn RTX-4090 A/Bs, 2026-06-04, `3c58f39`), with §H caveats (H9–H14). SVGF-real is intentionally `unsupported` on both converged backends (regime mismatch — they use `oidn-final`).
