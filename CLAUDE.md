...
- **B2 — RC into HybridEngine** — W8 sprint **in flight (2026-05-18)**: Phase 1A (cascade data types THREE-free) + Phase 1B (`RCDispatcher.dispatchFrameRaw` raw-GPU entry) + Phase 2 (`HybridEngineOptions.rcEnabled` toggle + per-engine `RCSubsystem`) all landed. Phase 3 (shade.wgsl Lo_rc sample + MIS composition with DDGI / ReSTIR-GI) + Phase 4 (reference renders + acceptance test) outstanding. See [plan/w8-rc-mis-composition.md](./plan/w8-rc-mis-composition.md).
