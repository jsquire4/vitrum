...
1. **`items_to_fix.md` remaining Section B item** — B2 (RC into HybridEngine) **shipped 2026-05-18** via W8 Phases 1A/1B/2/3/4. See the file at the repo root for verified-current locations. **A3 follow-up:** geometry-change topology rebuild ALREADY HANDLES geometry diffs (positions/normals/uvs/tangents/indices via full SAH rebuild in `HybridEnginePrimitiveUpdates.ts:topologyRebuild`). The "throws on geometry diffs" callout in older notes referred to the pre-A3 state and is stale. The remaining incremental-leaf optimisation (patch only affected BVH leaves, skip full SAH rebuild) is a perf follow-up, not a correctness gap.
2. **`plan/premium-grade-refactor-20260517.md` — all workstreams shipped except W5 + the W8 extraction follow-up.** Verified by git-log grep on 2026-05-18:
   - **W1** all 6 rounds done; **W2** done; **W3** D1/D2/D3/D4/D5/D6/D7/D8/D12/D13/D15/D16/D17/D18/D19 all merged; **W4** A4/A5/A7/A8/A9 + the HybridEngine decomp + WalkaroundGPUPipeline split + probeUpdatePass split all merged (A6 was never separately scoped); **W6** E1/E2/E3/E6 done; **W7** all sub-items done; **W8** RC into HybridEngine done end-to-end (Phases 1A/1B/2/3/4); **W9** PPG GPU dTree merged; **W10** neural-full-finish merged; **W11** OIDN wire done; **W12** dev overlays done; **W13** README/plan-archive done.
   - **Remaining: W5** (per-pass BGLs) — no commit landed under that name; needs scope verification against the W1-R5 pass-registry refactor that may have absorbed it. **W8 follow-up**: original W8 framing was "extract RC → `@vitrum/walkaround-rc` package"; the 2026-05-18 sprint reframed it as in-package wiring (`HybridEngineRC.ts` / `RCSubsystem`). The extraction would still be valuable post-Phase-4 acceptance.
