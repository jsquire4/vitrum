...
1. **`items_to_fix.md` remaining Section B item** — B2 (RC into HybridEngine) **shipped 2026-05-18** via W8 Phases 1A/1B/2/3/4. See the file at the repo root for verified-current locations. **A3 incremental-leaf BVH:** explicitly deferred. `HybridEnginePrimitiveUpdates.topologyRebuild` does a full ~50 ms SAH rebuild on geometry diffs (Option (a)). The Option (b) surgical leaf replacement (`rebuildBvhLeaf(bvh, leafIndex, newTriangles)` in shared-bvh) was rejected on cost-benefit grounds — three-mesh-bvh's `MeshBVH` constructor builds monolithically, so any partial rebuilder has to re-implement SAH partitioning. Topology changes are also rarer than transform / material edits, and the fast paths (`transformRefit` + material bytes-only re-upload) cover the common case. Verdict (2026-05-18): not worth the multi-sprint engineering until a host workload actually shows topology-edit frequency that justifies it. See `HybridEnginePrimitiveUpdates.ts:215-228` for the authoritative analysis.
