...
| `walkaround/useSceneBVH.ts` | Host hook (RC-specific signature). Not a duplicate of `walkaround/lib/useSceneBVH.ts` — the two have diverged into sibling hooks with different signatures and return types. Host-side concern. |
| `walkaround/lib/useSceneBVH.ts` | Sibling host hook (generalised signature). Use `@vitrum/shared-bvh` for any library BVH work. |
