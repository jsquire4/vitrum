...
- **BDPT (Sprint 10c)** — opt in via `extensions['vitrum.ptWebgl.bdpt'] = true` + `extensions['vitrum.ptWebgl.bdptMaxLightBounces']` (1–3). Per frame: `engine.fillBdptLightPath(bdpt, frameSeed)` then `engine.bdptAdvanceFrame(bdpt.texture)` before `renderFrame()`. Hardware GL runs the fork GPU light-subpath pass; SwiftShader uses CPU bounce-0 fill only. WSL Playwright often still reports SwiftShader for WebGL (~12 KiB BDPT PNGs); use `npm run benchmark:bdpt-layered-refs-gpu` (headed WSL Chromium) and rely on promotion guards — committed mechanical fixtures under `tools/reference-renders/bdpt-layered-mechanical/` were captured when hardware WebGL was active.
