...
// C10: Rec.709 luminance helper rec709Luminance is canonical (shared-samplers/luminance.wgsl).
...
fn svgf7x7FallbackMain(@builtin(global_invocation_id) gid: vec3u) {
  ...
  for (var dy: i32 = -3; dy <= 3; dy++) {
    for (var dx: i32 = -3; dx <= 3; dx++) {
      ...
      let lum = rec709Luminance(textureLoad(sfb_currColor, vec2u(p), 0).rgb);
      sumL  += lum;
      sumL2 += lum * lum;
      n     += 1u;
    }
