 ...
 *   binding 2 — texture_storage_2d<r32float, write> varianceOut   (variance)
 *
 * References:
...
@group(0) @binding(2) var vmom_varianceOut: texture_storage_2d<r32float, write>;
...
fn svgfVarianceFromMomentsMain(@builtin(global_invocation_id) gid: vec3u) {
  ...
  textureStore(vmom_varianceOut, gid.xy, vec4f(variance, 0.0, 0.0, 0.0));
}
`;
