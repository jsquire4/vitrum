 ...
 *
 * Module split (W4-A7 / sweep H8):
 *   - CPU emulation oracles  → ./svgfRealCpu.ts
 *   - Pipeline cache + WeakMap → ./svgfRealPipelineCache.ts
 *   - Texture upload helpers → ./webGpuTextureUpload.ts (shared with atrousVarianceWebGPU)
 *
 * References:
...
// Re-export CPU emulation oracles + their types from their new home for
// backward compatibility with existing tests + the package index barrel.
export {
  svgfReprojCPU,
