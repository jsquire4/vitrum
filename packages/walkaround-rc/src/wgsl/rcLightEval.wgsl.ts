 ...
 * RC_LIGHT_EVAL_WGSL is a string template inserted into PROBE_RAY_CAST_WGSL
 * at the position just after the bind-group declarations, so every binding
 * and struct the functions reference (rc_triMatId, rc_materials, rc_emitters,
 * rc_lights, RCLight, RCLightBuffer, RC_LIGHT_* consts, rcTraceFirstHit,
 * pcgHashToF32) is already declared before the composition point.
 *
 * Exported functions:
