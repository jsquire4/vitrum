...
/**
 * CPU mirror of `vitrumMeasureAreaVector` in the production GLSL.
 *
 * Geometry is quantized to the RGBA32F values the renderer consumes. A shared
 * max-component scale keeps the cross direction O(1); the requested area is
 * recovered as `coefficient * scaledCrossLength * edgeScale * edgeScale`.
 */
...
export function normalizeDirectionF32(value: Vec3): Vec3 | null {
