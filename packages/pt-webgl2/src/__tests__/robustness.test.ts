...
describe('pt-webgl2 robustness — non-throwing warnings and post-publication retirement', () => {
  ...
  it('publishes a full proposal-consistent environment replacement and retires every superseded texture', async () => {
    const gl = createMockGl();
    const engine = await createPTEngine_WebGL2({ device: gl });
    engine.setScene({
      ...smallScene(),
    ...
    expect(deleteTexture.mock.calls.length).toBeGreaterThan(2);
    expect(warnings).toContain('pt-webgl2.texture-retirement-failed');
    engine.dispose();
  });
});
