...
| Bidirectional path tracing (`bdpt: true`) | Supported (A5, 2026-06-10): host opt-in (`bdpt: true`); analytic-light subpath passes are driven on any driver. Mesh-area and environment sources remain on the unidirectional NEE/BSDF fallback path for BDPT connections. No ANGLE-specific gating exists — `EXT_disjoint_timer_query` is NOT used as a gate. |
...
- **BDPT** (`bdpt: true`): BDPT is implemented and host-driven (A5). Light subpaths sample analytic `Scene.emitters`; mesh-area and environment sources still contribute through the unidirectional NEE/BSDF path and emit a structured `pt-webgl2.bdpt-analytic-light-subpaths-only` warning when present. No ANGLE-specific gating exists — there is no `EXT_disjoint_timer_query` gate and no driver detection path. `bdpt: true` defaults to endpoint-only light-subpath depth (`bdptOptions.maxLightBounces: 1`), matching the safe-default proof posture. Explicit `bdptOptions.maxLightBounces > 1` opts into the current multi-vertex research path, capped at the WebGL2 backend's 3-column light-path texture with structured warnings.
