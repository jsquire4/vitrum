...
`createEngine()` picks a backend from `EngineOptions.prefer` (`'realtime' | 'quality' | 'auto'`) plus runtime probing:
...
Backend-specific tuning goes through `EngineOptions.extensions['walkaround-hybrid']` etc. — see each backend's options interface.
