  ...
  while (stackPtr > 0u) {
    ...
    if ((splitOrCount & LEAFNODE_FLAG) == LEAFNODE_FLAG) {
      ...
      for (var i = 0u; i < count; i = i + 1u) {
        ...
        // Local-ray distances are not directly comparable to world-space t due to
        // possible non-uniform instance scaling/shear. Use an unbounded local
        // query, then clamp with reconstructed worldDist below.
  ...
  while (stackPtr > 0u) {
    ...
    if ((splitOrCount & LEAFNODE_FLAG) == LEAFNODE_FLAG) {
      ...
      for (var i = 0u; i < count; i = i + 1u) {
        ...
        var localHit: SceneHit;
        let blasRoot = select(0u, tlasBlasRoots[instIdx], instIdx < arrayLength(&tlasBlasRoots));
        // Same conservative rule as traceTlasClosest: local t cannot safely use
        // world-space tMax under arbitrary instance transforms.
