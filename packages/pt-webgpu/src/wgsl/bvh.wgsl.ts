/**
 * BVH traversal + analytic-shape intersection — the closest-hit / any-hit
 * dispatch for the path tracer.
 *
 *  - `SceneHit` — common payload for closest and any-hit traversal
 *  - AABB / sphere / cylinder / capsule / H-channel local-space intersectors
 *  - `transformPointCols` / `transformDirectionCols` / `transformNormalFromWorldToLocalCols`
 *    — analytic-instance world<->local transforms (column-major)
 *  - `traceMeshBvh` — relative-offset stack BVH traversal (matches
 *    shared-bvh/normalizeBvhInteriorOffsets encoding)
 *  - `traceAnalyticShapes` — discriminated-union dispatch by SHAPE_* tag
 *  - `traceClosest` / `traceAny` — the two ray-query primitives the kernel uses
 *  - `hitMaterialId` — material-id lookup that bridges tri-index and analytic-index ranges
 *
 * Future (W2-C1): once shared-bvh exports a canonical `BVH_TRAVERSE_WGSL`
 * snippet usable by all backends, this file should re-export from there
 * instead of duplicating the traversal body.
 */
export const PT_WEBGPU_BVH_WGSL = /* wgsl */ `
