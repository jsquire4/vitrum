...
/**
 * Pick full vs lite from adapter limits. Full tier (23 buffers, 5 textures) enables
 * TLAS, analytics, HDRI, all emitter buffers, motion/variance aux, and caustics.
    ...
    if (tier !== 'full') {
      ...
      throw new Error(
        `pt-webgpu: traceTier=full requested but adapter reports ` +
          ...
          `maxStorageTexturesPerShaderStage=${maxTextures} (need ≥${PT_WEBGPU_REQUIRED_STORAGE_BUFFERS_PER_STAGE} and ≥5). ` +
