...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(387636); // re-pinned 2026-06-18 (H55 concentric-disc signed-denominator fix). See wgslContract.test.ts for the authoritative SHA.
