...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(365958); // re-pinned 2026-06-15 (ReSTIR-PT clearcoat-normal payload parity). See wgslContract.test.ts for the authoritative SHA.
