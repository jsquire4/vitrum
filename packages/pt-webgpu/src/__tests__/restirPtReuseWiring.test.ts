...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(303549); // re-pinned 2026-06-10: item 21 — spectral×photon-map gather fix (heroLambda param added to sppmGather + photonMapContribution; spectralEmissionAtHero applied to flux at gather time in spectral mode).
