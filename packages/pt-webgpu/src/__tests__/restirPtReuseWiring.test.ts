...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(372632); // re-pinned 2026-06-16 (SPPM stable-grid + overflow compensation). See wgslContract.test.ts for the authoritative SHA.
