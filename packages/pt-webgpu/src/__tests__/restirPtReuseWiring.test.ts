...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(314407); // re-pinned 2026-06-10: native analytic disc emitters (disc-area → rect stream + shape tag, concentric-disc sampling; 32-triangle fan removed). See wgslContract.test.ts for the authoritative SHA.
