...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(387611); // re-pinned 2026-06-18 (material descriptor constants now host-derived). See wgslContract.test.ts for the authoritative SHA.
