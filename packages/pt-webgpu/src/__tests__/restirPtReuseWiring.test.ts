...
describe('ReSTIR-PT reuse wiring — OFF by default (byte-identity)', () => {
  it('default-trace WGSL is untouched (the composed string is NOT modified by the reuse wiring)', () => {
    ...
    expect(PT_WEBGPU_TRACE_WGSL.length).toBe(311413); // re-pinned 2026-06-10: A4-progressive — SppmPixelStats binding(9), sppmGatherProgressive, pixelIndex in main (see wgslContract.test.ts for the authoritative SHA).
