...
- `<DenoiserABToggle>` — keyboard `D` / badge toggles `engine.debug.setDenoiserEnabled` on `@vitrum/walkaround-hybrid` (bypasses the denoiser pass; raw HDR). Backends without that method render an explicitly unavailable, non-interactive status with no keyboard shortcut.
