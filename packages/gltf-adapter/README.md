...
| Morph targets (POSITION + NORMAL + TANGENT deltas, sparse OK) | Supported → `SkinnedMeshPrimitive.morphTargets` / `.morphTargetNormals` / `.morphTargetTangents` / `.morphWeights` (node/mesh weights; unskinned morphed meshes get a synthesized identity skeleton). The shared CPU skin solver applies TANGENT deltas to solved tangent-space shading when rest tangents exist; compatibility remains approximate because GPU-native tangent skinning falls back to CPU. |
