...
compatible; image decoding is pluggable, with small built-in Node PNG/JPEG
...
| Morph targets (POSITION + NORMAL + TANGENT deltas, sparse OK) | Supported → `SkinnedMeshPrimitive.morphTargets` / `.morphTargetNormals` / `.morphTargetTangents` / `.morphWeights` (node/mesh weights; unskinned morphed meshes get a synthesized identity skeleton). The shared CPU skin solver applies TANGENT deltas to solved tangent-space shading when rest tangents exist; compatibility remains approximate because GPU-native tangent skinning falls back to CPU. Morph-target `TEXCOORD_*` deltas are not represented by the core Scene contract; import keeps rest-pose UVs and emits source-pathed `ignored-morph-target-texcoord` diagnostics plus `morphTargetTexcoords=unsupported` compatibility issues. |
