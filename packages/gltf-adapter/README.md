...
| Point/line modes (POINTS, LINES, LINE_LOOP, LINE_STRIP)                                                      | Imported as analytic `sphere` / `capsule` primitives (one sphere per point, one capsule per non-degenerate segment). Full `pt-webgpu` intersects them natively; `pt-webgpu-lite`, `walkaround-hybrid`, and `pt-webgl2` tessellate analytics and report `fallback-generated-mesh`. `reject-unsupported` accepts them; `reject-degraded` rejects only on tessellating backends. Radius order: `extras.vitrum.radius` on the primitive, mesh, or node, then host `pointLineFallbackRadius`, then a deterministic AABB-diagonal default. Vertex attributes, morphs, and skins are not applied.                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Morph targets (POSITION + NORMAL + TANGENT + TEXCOORD_N + COLOR_N, sparse OK)                               | Supported → scalable core morph streams plus compatibility aliases for UV sets 0/1 and color set 0. The shared solver blends every represented UV-set and additive RGB/RGBA color-set delta, and applies tangent deltas when rest tangents exist. A `TEXCOORD_N` or `COLOR_N` delta without a matching base stream rejects. Application-specific morph deltas are diagnosed with exact source paths and omitted; unknown non-application morph semantics reject.                                                                                                                                                                                                                                                                                                                                                                                                |
