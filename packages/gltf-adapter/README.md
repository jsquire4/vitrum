...
| EXT_mesh_gpu_instancing | Supported for mesh nodes → core `InstancedMeshPrimitive` with `nodeWorld * instanceTRS` baked into each instance matrix. Required use is accepted. `GltfSceneController` patches `instances[]` when the instanced node or an ancestor animates. Malformed accessors warn and import the base mesh once. Skinned/morphed instancing preflights as `EXT_mesh_gpu_instancing.skinnedOrMorphed=unsupported` under strict compatibility because core has no instanced skinned/morphed primitive contract yet; best-effort import keeps a structured warning and imports the skinned/morphed primitive once. |
| Morph targets (POSITION + NORMAL + TANGENT + TEXCOORD_0/1 deltas, sparse OK) | Supported → `SkinnedMeshPrimitive.morphTargets` / `.morphTargetNormals` / `.morphTargetTangents` / `.morphTargetUvs` / `.morphTargetUv1s` / `.morphWeights` (node/mesh weights; unskinned morphed meshes get a synthesized identity skeleton). The shared CPU skin solver applies TANGENT deltas to solved tangent-space shading when rest tangents exist and blends UV0/UV1 deltas into posed texture coordinates for backend upload. Compatibility remains approximate for tangent deltas because GPU-native tangent skinning falls back to CPU. Missing-base UV morphs and `TEXCOORD_2+` deltas still emit source-pathed `ignored-morph-target-texcoord` diagnostics plus `morphTargetTexcoords=unsupported` compatibility issues. |
