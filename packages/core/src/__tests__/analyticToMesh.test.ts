...
describe('analyticPrimitiveToMesh', () => {
  it('preserves id, material, transform, and hard-edged box extents', () => {
    ...
    const prim = analytic('box', [1, 2, 3, 4, 5, 6], { transform });
  ...
  it('generates deterministic smooth sphere geometry from options', () => {
    ...
    expect(a.positions.length / 3).toBe(26);
  ...
  it('generates cylinder geometry along the local Y axis', () => {
    ...
    expect(mesh.positions.length / 3).toBe(34);
