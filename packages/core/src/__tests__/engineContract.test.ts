...
describe('backend promise ledger', () => {
  ...
  it('pins the spot-checked material matrix rows derived from code reads (CAP-01)', () => {
    ...
    expect(gpu.clearcoatNormalMap).toBe('unsupported');
    ...
    expect(gpu.normalMap).toBe('approximate');
  ...
  it('keeps the shadow support matrix exhaustive and pinned for every backend (SHADOW-01)', () => {
    ...
    // (GI/DDGI/RC occlusion ignores it); emitter flag is honored by direct,
    // area-emitter, DDGI fixture/sun, RC fixture/sun, and direct-sun paths.
