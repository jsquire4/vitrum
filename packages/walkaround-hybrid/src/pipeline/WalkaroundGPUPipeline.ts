 ...
 * / B5 / B6).
 *
 * Primary visibility is generated by the RIS + shade BVH ray casts. The frame
 * bind-group layout therefore begins at binding 5 and carries no draft
 * G-buffer inputs.
 */
...
import { compilePipelines, createCompositePipeline } from './pipelineCompiler.js';
...
import {
  ...
  resolveNrcConfig,
  type NrcConfig,
  type NrcLearnedStateImportTransaction,
 ...
 * this handle is returned. `commit()` publishes without retiring the previous
 * generation, `rollback()` restores it, and `finalize()` makes the publication
 * irreversible by retiring whichever generation is no longer live.
 */
export interface ReservoirImportTransaction {
  commit(): void;
  rollback(): void;
  finalize(): void;
}

export type RestirGIReservoirImportTransaction = ReservoirImportTransaction;
export type RestirDIReservoirImportTransaction = ReservoirImportTransaction;

interface RestirGIHistoryEpochInspection {
  readonly valid: boolean;
  /** Null means that the snapshot contains no semantically live GI record. */
 ...
 * of the imported history reusable after publication.
 */
function inspectRestirGIHistoryEpoch(
  snapshot: RestirGISnapshot,
): RestirGIHistoryEpochInspection {
  let epoch: number | null = null;
  const recordCount = snapshot.halfW * snapshot.halfH;
  for (const data of [
    snapshot.current,
    snapshot.previous,
    snapshot.spatial,
    ...
    for (let record = 0; record < recordCount; record += 1) {
      ...
      if (recordEpoch === 0 || (epoch != null && epoch !== recordEpoch)) {
        return { valid: false, epoch: null };
      }
      epoch = recordEpoch;
    }
  }
  return { valid: true, epoch };
}
...
interface RegisterPassesDeps {
  ...
  varianceTrackerPingPongRef: PingPongRef;
  ...
  shouldDispatchVarianceTracker: () => boolean;
  getRegirResources: () => {
...
function registerPasses(
  ...
  registry.register(new SpatialReservoirPass(
    compiled.spatialPipeline,
    compiled.spatialPipelineRoundTwo,
    deps.diSpatialPasses,
  ));
  ...
  registry.register(
    new VarianceTrackerPass(
      deps.varianceTrackerPingPongRef,
      deps.shouldDispatchVarianceTracker,
    ),
  );
  ...
  registry.register(
    new IndirectTemporalAccumPass(
      ...
      compiled.indirectTemporalAccumResetPipeline,
    ),
  );
...
export class WalkaroundGPUPipeline implements BvhUpdateSink {
  ...
  /**
   * Fail-safe reset barrier for every temporal consumer. It is independent of
   * algorithm-specific history: DI, canonical generalized-reuse GI, denoisers,
   * accumulation, checkerboard reprojection, and camera-only motion vectors all
   * share it.
   */
  private _temporalHistoryClearPending = true;
  /**
   * Disables checkerboard dispatch for one successful frame after any temporal
   * reset or history import. Imports preserve their reservoirs, so this flag is
   * deliberately separate from `_temporalHistoryClearPending`: the first
   * post-import frame must initialize non-persisted radiance/denoiser history
   * at full rate without erasing the restored DI/GI cohort.
   */
  private _temporalHistoryFullRatePending = true;
  ...
  *  through a {@link PingPongRef} wrapper. */
  ...
  /** Welford history for modes whose denoiser does not produce it itself. */
  private _varianceTrackerPingPongRef: PingPongRef = { value: 0 };
  ...
  get gpuMemoryExternalSections(): GpuMemoryExternalSections {
    ...
    const neuralBytes =
      this._inferenceGraph?.memoryTelemetry?.totalAllocatedBytes ?? 0;
    const nrcBytes =
      this._nrc?.diagnostics().persistentBufferBytes ?? 0;
    return {
      ...this._bvhHost.gpuMemorySections(),
      ...this._ddgi.gpuMemorySections(),
      ...(neuralBytes > 0
        ? {
            neural: {
              // Graph allocations include tensors, parameters, uniforms, and
              // placeholders. They are graph-owned rather than exposed as
              // FrameResources, so represent the exact telemetry total as one
              // storage-class external allocation.
              graphAllocation: {
                size: neuralBytes,
   ...
   */
  getMemoryBreakdown(
    additionalSections: GpuMemoryExternalSections = {},
  ): GpuMemoryBreakdown | null {
    if (!this._initialized) return null;
    return estimateFrameResourcesMemory(this._res, {
      ...this.gpuMemoryExternalSections,
      ...additionalSections,
    });
  }
  ...
  canImportRestirGIReservoirs(snap: RestirGISnapshot): boolean {
    ...
    if (
      ...
      snap.halfW !== halfW ||
      snap.halfH !== halfH ||
      snap.strideU32 !== RESERVOIR_GI_STRIDE_U32
    ) {
    ...
    return (
      ...
      inspectRestirGIHistoryEpoch(snap).valid
    );
  }
  ...
  prepareRestirGIReservoirImport(
    ...
    const history = inspectRestirGIHistoryEpoch(snap);
    if (!history.valid) return null;
    const liveResources = this._res.restirGI;
    const replacement = this.#prepareReservoirReplacement(
      device,
      'reservoir-gi',
      () => {
        const r = this._res.restirGI;
        return [
          r.reservoirGiCurrentBuffer,
          r.reservoirGiPreviousBuffer,
          r.reservoirGiSpatialBuffer,
        ] as const;
      },
      ([current, previous, spatial]) => {
        this._res.restirGI = Object.freeze({
      ...
      () => {
        this._res.restirGI = liveResources;
      },
      [snap.current, snap.previous, snap.spatial],
    );
    return this.#prepareRestirGIHistoryPublication(
      replacement,
      history.epoch,
    );
  }
  ...
  /**
   * Prepare an all-zero live-layout cohort for migration from a valid compact
   * snapshot. Allocation and publication use the same transaction as a regular
   * restore; DDGI can therefore commit atomically with a deliberately cold
   * high-frequency ReSTIR history.
   */
  prepareColdRestirGIReservoirImport(
    device: GPUDevice,
    halfW: number,
    halfH: number,
  ): RestirGIReservoirImportTransaction | null {
    ...
    return this.#prepareRestirGIHistoryPublication(replacement, null);
  }

  /**
   * Couple a GI buffer replacement to the CPU-side temporal control state.
   * The imported epoch is published atomically with the buffers and every
   * non-persisted radiance history is cold-started on the next full-rate frame.
   */
  #prepareRestirGIHistoryPublication(
    replacement: ReservoirImportTransaction,
    importedEpoch: number | null,
  ): ReservoirImportTransaction {
    const previousAccumFrame = this._accumFrameIndex;
    const previousGrisEpoch = this._grisHistoryEpoch;
    const previousClearPending = this._temporalHistoryClearPending;
    const previousFullRatePending = this._temporalHistoryFullRatePending;
    ...
    return {
      commit: () => {
        ...
        this._grisHistoryEpoch = importedEpoch ?? previousGrisEpoch;
        this._temporalHistoryClearPending = false;
        this._temporalHistoryFullRatePending = true;
        state = 'committed';
      },
      rollback: () => {
        ...
        if (state === 'committed') {
          ...
          this._temporalHistoryClearPending = previousClearPending;
          this._temporalHistoryFullRatePending = previousFullRatePending;
        }
        state = 'closed';
        if (rollbackErrors.length > 0) {
          throw new AggregateError(
            rollbackErrors,
            'ReSTIR-GI temporal-state rollback failed.',
          );
        }
      },
      finalize: () => {
  ...
  #prepareReservoirReplacement(
    device: GPUDevice,
    labelPrefix: string,
    getLive: () => readonly [GPUBuffer, GPUBuffer, GPUBuffer],
    publish: (
      buffers: readonly [GPUBuffer, GPUBuffer, GPUBuffer],
    ) => void,
    restoreLive: () => void,
    data:
      | readonly [Uint32Array, Uint32Array, Uint32Array]
      | null,
  ): ReservoirImportTransaction {
    const live = getLive();
    const size = live[0].size;
    const usage =
      GPUBufferUsage.STORAGE |
      GPUBufferUsage.COPY_SRC |
      GPUBufferUsage.COPY_DST;
    const candidates: GPUBuffer[] = [];
    const liveBuffers = new Set<GPUBuffer>(live);
    try {
      const createCandidate = (
        label: string,
        upload: Uint32Array | null,
      ): GPUBuffer => {
        const candidate = device.createBuffer({
          label,
          size,
          usage,
          mappedAtCreation: upload != null,
        });
        ...
        if (
          ...
          throw new Error(
            'Reservoir import candidate aliases a live or sibling buffer.',
          );
        }
        if (upload != null) {
          ...
          if (mapped.byteLength !== size) {
            throw new Error(
              'Reservoir import candidate exposed an unexpected mapped size.',
            );
          }
          new Uint32Array(mapped).set(upload);
          candidate.unmap();
        }
        return candidate;
      };
      const current = createCandidate(
        `${labelPrefix}-current.import-candidate`,
        data?.[0] ?? null,
      );
      const previous = createCandidate(
        `${labelPrefix}-previous.import-candidate`,
        data?.[1] ?? null,
      );
      const spatial = createCandidate(
        `${labelPrefix}-spatial.import-candidate`,
        data?.[2] ?? null,
      );
      const replacement = [current, previous, spatial] as const;
      let state: 'prepared' | 'committed' | 'closed' = 'prepared';
      ...
      return {
        commit: () => {
          if (state !== 'prepared') return;
          const currentLive = getLive();
          if (
            currentLive[0] !== live[0] ||
            currentLive[1] !== live[1] ||
            currentLive[2] !== live[2]
          ) {
            throw new Error(
              'Reservoir import commit rejected because the live generation changed after preparation.',
            );
          }
          try {
            publish(replacement);
            ...
            if (rollbackErrors.length > 0) {
              throw new AggregateError(
                ...
                'Reservoir import commit failed and rollback also failed.',
              );
            }
            throw primaryError;
          }
        ...
        rollback: () => {
          ...
          if (rollbackErrors.length > 0) {
            throw new AggregateError(
              rollbackErrors,
              'Reservoir import rollback failed.',
            );
          }
        },
        finalize: () => {
          ...
          for (const buffer of liveBuffers) {
            try {
              ...
              // Publication is complete; old-generation retirement is best-effort.
            }
          }
        },
      };
  ...
  /** Read back one complete full-resolution ReSTIR-DI reservoir generation. */
  async exportRestirDIReservoirs(
    device: GPUDevice,
  ...
  /** Non-mutating semantic and live-layout compatibility probe for ReSTIR-DI. */
  canImportRestirDIReservoirs(snap: RestirDISnapshot): boolean {
    if (
      !this._initialized ||
    ...
    const r = this._res.restirDI;
    const buffers = [
      r.reservoirCurrentBuffer,
      r.reservoirPreviousBuffer,
      r.reservoirSpatialBuffer,
    ] as const;
    if (
      new Set<GPUBuffer>(buffers).size !== buffers.length ||
      buffers[0].size <= 0 ||
      buffers[0].size !== buffers[1].size ||
      buffers[0].size !== buffers[2].size ||
      buffers[0].size % Uint32Array.BYTES_PER_ELEMENT !== 0
    ) {
      return false;
    }
    const expectedU32 = buffers[0].size / Uint32Array.BYTES_PER_ELEMENT;
    return (
      snap.current.length === expectedU32 &&
      snap.previous.length === expectedU32 &&
      snap.spatial.length === expectedU32
    );
  }

  /** Prepare a reversible replacement of all three ReSTIR-DI buffers. */
  prepareRestirDIReservoirImport(
    device: GPUDevice,
    snap: RestirDISnapshot,
  ): RestirDIReservoirImportTransaction | null {
    if (
      device !== this._device ||
      !this.canImportRestirDIReservoirs(snap)
    ) {
      return null;
    }
    const liveResources = this._res.restirDI;
    return this.#prepareReservoirReplacement(
      device,
      'reservoir-di',
      () => {
        const r = this._res.restirDI;
      ...
      ([current, previous, spatial]) => {
        this._res.restirDI = Object.freeze({
          reservoirCurrentBuffer: current,
          reservoirPreviousBuffer: previous,
          reservoirSpatialBuffer: spatial,
        });
      },
      () => {
        this._res.restirDI = liveResources;
      },
      [snap.current, snap.previous, snap.spatial],
    );
  }

  /** One-shot ReSTIR-DI restore for callers outside a complete-state cohort. */
  importRestirDIReservoirs(device: GPUDevice, snap: RestirDISnapshot): boolean {
    const transaction = this.prepareRestirDIReservoirImport(device, snap);
    if (transaction == null) return false;
    try {
      transaction.commit();
      transaction.finalize();
  ...
  /** Whether complete-state persistence must carry a live PPG guide section. */
  get ppgStateRequired(): boolean {
    return this._initialized && this._ppg.enabled;
  }

  /** Whether complete-state persistence must carry a live NRC learned section. */
  get nrcStateRequired(): boolean {
    return this._initialized && this._nrc !== null;
  }
  ...
  canImportPPGSTree(snapshot: PPGSTreeSnapshot): boolean {
    return this._initialized && this._ppg.canImportSTree(snapshot, this._res);
  }

  preparePPGSTreeImport(
    snapshot: PPGSTreeSnapshot,
  ): PreparedSTreeImport | null {
    if (!this._initialized) return null;
    return this._ppg.prepareSTreeImport(snapshot, this._res);
  }
  ...
  exportNrcLearnedState(): Promise<NrcLearnedStateSnapshot | null> {
    return this._nrc?.exportLearnedState() ?? Promise.resolve(null);
  }

  canImportNrcLearnedState(
    snapshot: NrcLearnedStateSnapshot,
  ): boolean {
    return this._nrc?.canImportLearnedState(snapshot) ?? false;
  }

  prepareNrcLearnedStateImport(
    encoder: GPUCommandEncoder,
    snapshot: NrcLearnedStateSnapshot,
  ): NrcLearnedStateImportTransaction | null {
    return this._nrc?.prepareLearnedStateImport(encoder, snapshot) ?? null;
  }
    ...
    try {
      ...
      if (mapped) {
        try {
          ...
          // Preserve the readback result/failure.
        }
      }
      try {
        staging.destroy();
  ...
  /** Freshest Welford side for both denoiser-owned and shared tracking. */
  #currentWelfordPing(): number {
    if (
      this._denoiserPassEnabled &&
      this._activeDenoiser?.id === 'atrous-variance'
    ) {
      return this._activeDenoiser.welfordPing ?? 0;
    }
    return this._varianceTrackerPingPongRef.value;
  }
  ...
  async initialize(
    ...
    options?: {
       ...
       *  runtime UBO flag binding a fifth group on the NRC-off path would repeat
       *  the prior structural-layout regression (f8df9a4). Full-tier-only (the
       *  ctor forbids `tier:'lite' + nrcEnabled`). */
      nrcEnabled?: boolean;
   ...
   *   - checkerboard / NRC structural gate resolution (must precede compile)
  ...
  async #initGpuResources(
    ...
    // and gi-ris compiles + dispatches the 4-group DDGI pass.
    if (options?.nrcEnabled === true) {
  ...
  async #initPasses(
    ...
    const { registry, compositePass } = registerPasses(compiled, {
      ...
      varianceTrackerPingPongRef: this._varianceTrackerPingPongRef,
      regir: this._regir,
      bglCache: this._bglCache,
      bvhBuffers,
      nrcClearSlotClaims:
  ...
  prepareEmitterLightingMutation(
    ...
    const accumulatorMutation: PreparedSceneMutation = {
      ...
      rollback: () => {
        ...
        this._accumFrameIndex = previousAccumFrame;
        this._grisHistoryEpoch = previousGrisEpoch;
        this._temporalHistoryClearPending = previousTemporalClearPending;
        this._temporalHistoryFullRatePending =
  ...
  prepareSceneMutation(
    ...
    // temporal consumers even if a caller omitted requestAccumReset().
    const temporalHistoryAffected =
      sceneMutationRequiresTemporalReset(mutation);
  ...
  /**
   * Explicitly retry a PPG training epoch after the coordinator entered its
   * durable failed state. Returns false when PPG is disabled, not failed, or
   * the pipeline has already been torn down.
   */
  ...
  #buildFrameContext(
    ...
    const {
      frame: bgFrame,
      diSpatialReverse: bgDiSpatialReverse,
    ...
    const diTerminalFrameBindGroup =
      this._diSpatialPasses === 1 ? bgFrame : bgDiSpatialReverse;
    const diTerminalReservoirBuffer =
      this._diSpatialPasses === 1
        ? this._res.restirDI.reservoirSpatialBuffer
    ...
    if (this._temporalHistoryClearPending) {
      clearTemporalReservoirHistory(encoder, this._res);
    }
    ...
    const passCtx: PassDispatchContext = {
      ...
      diSpatialReverseFrameBindGroup: bgDiSpatialReverse,
      diTerminalFrameBindGroup,
      diTerminalReservoirBuffer,
      ...
      // Non-atrous denoisers use the pipeline-wide tracker; atrous-variance
      // keeps its denoiser-owned producer and exposes the same side index.
      welfordPing: this.#currentWelfordPing(),
      gtaoDownscale: this._gtaoDownscale,
      gNormalDepthView,
      computeDesc,
  ...
  #dispatchPasses(
    ...
    publication.stage(() => {
      ...
      this._temporalHistoryClearPending = false;
      this._temporalHistoryFullRatePending = false;
    });
    ...
    encoder.copyBufferToBuffer(
      passCtx.diTerminalReservoirBuffer,
      0,
      this._res.restirDI.reservoirPreviousBuffer,
      0,
      passCtx.diTerminalReservoirBuffer.size,
    );
    encoder.copyBufferToBuffer(
