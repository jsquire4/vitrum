...
function makePipeline(submitImpl?: () => void): {
  ...
  state._temporalHistoryClearPending = false;
...
describe('temporal history mutation boundary', () => {
  it('invalidates all temporal consumers without retaining a retired structural selector', () => {
    const { pipeline, state } = makePipeline();
    expect('_grisReuseStructural' in pipeline).toBe(false);
    ...
    expect(state._temporalHistoryClearPending).toBe(true);
  });

  it('invalidates a geometry mutation', () => {
    const { pipeline, state } = makePipeline();
    state._grisHistoryEpoch = 9;
    state._temporalHistoryClearPending = false;
    state._accumFrameIndex = 13;
    ...
    expect(state._accumFrameIndex).toBe(0);
    expect(state._grisHistoryEpoch).toBe(10);
    expect(state._temporalHistoryClearPending).toBe(true);
  });
  ...
  it('makes emitter publication epoch invalidation transactional', () => {
    ...
    expect(state._temporalHistoryClearPending).toBe(false);
  });

  it('defers the all-reservoir clear to the first post-mutation frame', () => {
    ...
    expect(gpu.encoder.clearBuffer).not.toHaveBeenCalled();
    prepared.commit();
  ...
  it('rolls epoch, pending-clear, and accumulator state back when clear submit fails', () => {
    ...
    state._temporalHistoryClearPending = true;
