...
function makeFrameDeps(opts: {
  ...
  const stubDdgi = {
    ...
    pass: { setGlassMixScale: () => undefined, getReadAtlasGPUTextures: () => null },
    probeGrid: { params: {} },
  };
  return {
    state: 'ready',
    debug: false,
    dbg: null,
    pipeline: stubPipeline as unknown as HybridEngineFrameDeps['pipeline'],
    bvhBuffers: { totalEmissivePower: 1, emitters: { count: 0 }, bvhMode: 'merged' } as unknown as HybridEngineFrameDeps['bvhBuffers'],
    ...
    ddgiOn: true,
    isLayerEnabled: () => true,
    ddgi: stubDdgi as unknown as HybridEngineFrameDeps['ddgi'],
    ...
    device: makeStubDevice(),
    tunables: { glassMixScale: 1, triIntersectEpsilon: 1e-5 } as unknown as HybridEngineFrameDeps['tunables'],
    rc: null,
    rcWeight: 0,
    primaryLightDir: [0, -1, 0],
    primaryLightIntensity: 1,
    ...
    verbose: false,
    debugTimings: [],
    debugSurface: { estimatedGpuMemoryBytes: () => undefined } as unknown as HybridEngineFrameDeps['debugSurface'],
    presentLastFrame: () => undefined,
  };
}
