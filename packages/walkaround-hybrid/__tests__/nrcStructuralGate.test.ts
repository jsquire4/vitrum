 ...
 * (restirPtReuse) added a `@group(1)` bind group + a two-group pipeline layout
 ...
 *
 * NRC (Müller et al. 2021) adds, when ON, a `@group(4)` NRC bind group (MLP
 * weights/biases + hash-grid tables + level descs + record gather + config UBO)
 * + the inline-MLP-forward shader variant to the gi-ris pass. This test asserts
 ...
 *      text with NO `@group(4)` and NONE of the NRC identifiers (the NRC bind
 ...
 *      pipeline layout with exactly FOUR bind-group layouts when OFF and FIVE
 *      when ON, and the SET of compiled pipeline labels (the registered-pass
 *      reflection) is IDENTICAL between OFF and ON — no NRC-only pipeline is
 *      added, no pass is removed. The ONLY structural delta is the gi-ris
 *      bind-group count, exactly as intended.
 */
...
// @group(4) bindings + the inline-MLP/query/record/spread symbols that the NRC
// variant introduces.
const NRC_IDENTS = [
  '@group(4)',
  'nrcWeights',          // the @group(4) MLP weight binding
  'nrcBiases',
  'nrcTables',
  'nrcRecords',          // the record-gather binding
...
describe('gi-ris SHADER structure — default (nrcEnabled OFF) is pre-NRC', () => {
  it('OFF gi-ris composes with NO @group(4) and NO NRC identifiers', () => {
  ...
  it('ON gi-ris composes WITH @group(4) and the NRC identifiers', () => {
...
describe('gi-ris PIPELINE LAYOUT + registered-pass set — gated at compile time', () => {
  ...
  it('opt-in (nrcEnabled ON): risGi uses a FIVE-group layout', async () => {
    const recorded = await compileAndCollect(true);
    const byLabel: Record<string, number> = {};
    for (const r of recorded) byLabel[r.label] = r.bglCount;
    expect(byLabel.risGi, 'NRC risGi pipeline must have 5 bind-group layouts').toBe(5);
  });
  ...
  it('the SET of compiled pipelines is IDENTICAL OFF vs ON (no pipeline/pass added or removed)', async () => {
    ...
    // The ONLY structural delta is the risGi bind-group COUNT (4 → 5), not the
    // set of pipelines compiled. NRC adds NO new compute/render pipeline to the
