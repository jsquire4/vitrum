...
- **Approximate material lobes** — atlas-backed scalar `specular`, `clearcoat`, and `sheen` controls feed shade-owned direct, analytic, sun, and glossy-indirect lighting; readable specular, clearcoat factor/roughness, and sheen color/roughness maps multiply the scalar controls. These rows remain approximate because upstream ReSTIR candidate PDFs/payloads and clearcoat-normal/remaining extension maps are not lobe-complete.
