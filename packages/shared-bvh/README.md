...
- `fingerprintBuffer` / `fingerprintBuffers` — sampled large-buffer fingerprints for versioning and upload heuristics; `fingerprintBufferExact` / `fingerprintBuffersExact` — full-byte fingerprints for correctness-sensitive rebuild-skip gates such as `SceneBvh`.
