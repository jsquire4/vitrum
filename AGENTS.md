...
- E6 — **historical / superseded by fork removal.** The old fork-backed `packages/pt-webgl/src/forkAccess.ts` row is not a live target because `@vitrum/pt-webgl` was removed; the active native WebGL2 backend is `packages/pt-webgl2`.

### W7 — dead-code + misplaced-code cleanup

- Dead-code batch (G2/G3/G5/G7/G9 — all in HEAD as of 2026-05-19) (`feat/w7-dead-code-batch`): deleted deprecated `bdptConnectionMIS_partial` / `buildBDPTStrategyPDFs_partial` (G2 ✓); `rgbToApproxSpectralCoefficients` aliasing (G3 ✓); shared-bvh export hygiene was reconciled, and `validateBvhEncoding` is now intentionally retained as an exported diagnostic helper per the H34 pack/refit safety closure; un-exported internal helpers `extractAttribute`/`extractIndex`/`warnOnce` from three-bindings (G7 ✓); deleted redundant emitter-constants re-exports from `uploadSceneBuffers` (G9 ✓).
- Misplaced-code moves (H1/H4/H5/H7/H8) (`feat/w7-misplaced-code-moves`): albedo demodulate/remodulate extracted to its own file (H8, `84f474b`); BDPT bounce-budget constants were moved during the retired fork era and the active native WebGL2 path now lives under `packages/pt-webgl2`; Halton-Shoemake quaternion sampler moved to shared-samplers (H5, `bae8066`); plus H1, H4.
